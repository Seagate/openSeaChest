...
if errorlevel 1 (
	...
	echo "regular build"
	call cmake -G "%string%" -A "%~2" -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DDISABLE_EXTRA_LIBS=ON -DBUILD_TESTING=OFF -DBUILD_APPS=OFF -DDISABLE_BSYMBOLIC=ON -DCMAKE_POLICY_DEFAULT_CMP0126=NEW -DCMAKE_POLICY_DEFAULT_CMP0156=NEW -DCMAKE_POLICY_DEFAULT_CMP0179=NEW -DDISABLE_WERROR=ON -DCMAKE_TRY_COMPILE_TARGET_TYPE=EXECUTABLE -S %~3\subprojects\json-c\ -B "%~3\subprojects\json-c\build-%~4-%~2-%~5"
) else (
	:: Performing static build 
	echo "static build"
	call cmake -G "%string%" -A "%~2" -DCMAKE_BUILD_TYPE=Release -DCMAKE_POLICY_DEFAULT_CMP0091=NEW -DCMAKE_MSVC_RUNTIME_LIBRARY="MultiThreaded$<$<CONFIG:Debug>:Debug>" -DBUILD_SHARED_LIBS=OFF -DDISABLE_EXTRA_LIBS=ON -DBUILD_TESTING=OFF -DBUILD_APPS=OFF -DDISABLE_BSYMBOLIC=ON -DCMAKE_POLICY_DEFAULT_CMP0126=NEW -DCMAKE_POLICY_DEFAULT_CMP0156=NEW -DCMAKE_POLICY_DEFAULT_CMP0179=NEW -DDISABLE_WERROR=ON -DCMAKE_TRY_COMPILE_TARGET_TYPE=EXECUTABLE -S %~3\subprojects\json-c\ -B "%~3\subprojects\json-c\build-%~4-%~2-%~5"
)
