...
// ******************************************************************************************
//
// \file openSeaChest_PassthroughTest.c CLI tool that attempts to determine what works or doesn't work to talk
// passthrough to a device
...
typedef struct s_passthroughTestParams
{
    ...
    bool disableTranslatorPassthroughTesting;  // Can be used if wanting to disable SAT or VS NVMe passthrough command
                                               // tests.
    bool allowLegacyATAPTTest;
    bool testPotentiallyDeviceHangingCommands; // extra testing for other commands that are known to cause issues on
                                               // some devices
    struct
    {
...
static void print_Drive_Type_Hint_Help(bool shortHelp)
{
    ...
    if (!shortHelp)
    {
        print_str("\t\tThis option passes a hint to the software that the device being tested is\n");
        print_str("\t\tan ATA or NVMe drive so it can appropriately test it.\n");
        print_str("\t\tThis option is most useful when also using --ptTypeHint\n");
        print_str("\n");
    }
}
...
static void print_Passthrough_Type_Hint_Help(bool shortHelp)
{
    ...
    if (!shortHelp)
    {
        print_str("\t\tAdd this option if the device being tested is likely to support one of\n");
        print_str("\t\tthe supported passthrough types. This hints to the software to perform\n");
        print_str("\t\tadditional testing for these passthroughs if no other support is apparent.\n");
        print_str("\t\t    sat - device supports SAT ATA-passthrough commands (12 or 16 byte)\n");
        print_str("\t\t    legacyATA - device supports a legacy vendor unique method to passthrough\n");
        print_str("\t\t                ATA commands. You must also specify --enableLegacyATAPTTest\n");
        print_str("\t\t                in order for these commands to be tested.\n");
...
static void print_Disable_PT_Testing_Help(bool shortHelp)
{
    ...
    if (!shortHelp)
    {
        print_str("\t\tDisables all ATA passthrough testing. Device will only be tested with SCSI commands from\n");
        print_str("\t\tSPC and SBC specifications.\n\n");
    }
}
...
static void print_Enable_Legacy_ATA_PT_Testing_Help(bool shortHelp)
{
    ...
    if (!shortHelp)
    {
        print_str("\t\tAdd this option to the command line to allow sending legacy passthrough CDBs from\n");
        print_str("\t\told USB adapters or drivers. By default these are off since these operation codes\n");
        print_str("\t\tmay perform unintended actions on devices that don't actually support passthrough.\n");
        print_str("\t\tFor example: There is a USB thumbdrive that receives one of these and bricks immediately.\n");
        print_str("\t\tOnly add this to the command line if you understand and have high confidence that the\n");
        print_str("\t\tdevice you are testing is likely to support one of these passthroughs. Most of these devices\n");
        printf(
...
static void print_Enable_Hang_Commands_Test_Help(bool shortHelp)
{
    ...
    if (!shortHelp)
    {
        ...
        printf("\t\tThis must be enabled manually for a more complete test, but if it hangs the device could cause "
               "problems.\n");
        printf("\t\tEach of these tests has a specific hack/quirk related to it, so if the device hangs, then it must "
               "be added\n");
        printf(
        ...
        printf("\t\t    rrTdir - in the SAT return response information protocol, run it without setting the tdir bit "
               "as the spec allows\n");
        print_str("\n");
    }
}
...
static void print_Force_Retest_Help(bool shortHelp)
{
    ...
    if (!shortHelp)
    {
        print_str("\t\tUse this option to force this utility to retest this device if it reports\n");
        print_str("\t\tthat it is already known. This may be useful when testing multiple devices\n");
        print_str("\t\tthat use the same chip and are identified, or when trying to troubleshoot\n");
        print_str("\t\tcompatibility on another OS.\n\n");
    }
}
...
static void print_Run_Passthrough_Test_Help(bool shortHelp)
{
    ...
    if (!shortHelp)
    {
        print_str("\t\tThis option is used to perform the passthrough test.\n");
        print_str("\t\tThe passthrough test is an attempt to figure out quirks or\n");
        print_str("\t\thacks with different translators in order to make the device\n");
        print_str("\t\tmore compatible with the rest of the openSeaChest software.\n");
        print_str("\t\tThe default test is done using only what can be determined about\n");
        print_str("\t\tthe device from the SCSI reported data. Attempting to passthrough\n");
        print_str("\t\tATA or NVMe commands may not be done depending on how the device reports.\n");
        print_str("\t\tUsing the other options can help tell this software to test for other\n");
        print_str("\t\tbehavior. The other options to control the test are as follows:\n");
        printf("\t\t    --%s\n", PT_DRIVE_HINT_LONG_OPT_STRING);
        printf("\t\t    --%s\n", PT_PTTYPE_HINT_LONG_OPT_STRING);
        printf("\t\t    --%s\n", DISABLE_PT_TESTING_LONG_OPT_STRING);
        printf("\t\t    --%s\n", ENABLE_LEGACY_ATA_PT_TESTING_LONG_OPT_STRING);
...
int main(int argc, char* argv[])
{
    ...
    while (1) // changed to while 1 in order to parse multiple options when longs options are involved
    {
        ...
        case 0:
            ...
            {
                if (strcmp(optarg, "ata") == 0 || strcmp(optarg, "ATA") == 0 || strcmp(optarg, "sata") == 0 ||
                    strcmp(optarg, "SATA") == 0 || strcmp(optarg, "pata") == 0 || strcmp(optarg, "PATA") == 0 ||
                    strcmp(optarg, "ide") == 0 || strcmp(optarg, "IDE") == 0)
                {
            ...
            else if (strcmp(longopts[optionIndex].name, PT_PTTYPE_HINT_LONG_OPT_STRING) == 0)
            {
                ...
                else if (strcmp(optarg, "legacyATA") ==
                         0) // TODO: add in ability to specify specific legacy ATA passthrough types
                {
                    // This means test all legacy USB ATA translations
                    PT_PTTYPE_HINT = 50; // setting this until I figure out a better way to do this
                }
                // TODO: Add vendor unique NVMe types here. Each one should be unique with some sort of generic value to
                // say "test all of them"
                else
                {
                    print_Error_In_Cmd_Line_Args(PT_DRIVE_HINT_LONG_OPT_STRING, optarg);
                    exit(UTIL_EXIT_ERROR_IN_COMMAND_LINE);
...
static eReturnValues return_Response_Extend_Bit_Test(tDevice* device)
{
    ...
    if (device->drive_info.ata_Options.fourtyEightBitAddressFeatureSetSupported)
    {
        ...
        {
            // warning that this device doesn't support a featureset that can be used to figure out if the extend bit is
            // reported properly.
            commandIssued = false;
            print_str("WARNING: Unable to perform RTFR test for extend bit with return response information.\n");
            print_str("         Please use a 48bit drive that supports either the HPA feature or AMAC feature\n");
        ...
        if (commandIssued)
        {
            ...
            {
                // This likely means we got everything, but need to confirm. It could also mean we think we got
                // everything, but we could have missed ext RTFRs too.
                if (nativeMaxLBA >= device->drive_info.bridge_info.childDeviceMaxLba &&
                    device->drive_info.lastCommandRTFRs.secCntExt != 0xFF &&
                    device->drive_info.lastCommandRTFRs.lbaLowExt != 0xFF &&
                    device->drive_info.lastCommandRTFRs.lbaMidExt != 0xFF &&
                    device->drive_info.lastCommandRTFRs.lbaHiExt != 0xFF)
                {
                    if (device->drive_info.passThroughHacks.ataPTHacks.returnResponseIgnoreExtendBit)
                    {
                        set_Console_Colors(true, HACK_COLOR);
                        print_str("HACK FOUND: RSIE\n");
                        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                    }
                    else
                    {
                        print_str("RTFRs returned correctly and extend bit appears to work properly.\n");
                    }
                    ret = SUCCESS;
                }
                ...
                else
                {
                    ...
                    printf("         This means only 28bit commands will get full results. 48bit commands will be "
                           "partial\n");
                    set_Console_Colors(true, HACK_COLOR);
            ...
            {
                // This COULD mean that the RTFRs didn't come back all the way, OR it could mean that the extend bit
                // should be ignored for return response info
                if (!device->drive_info.passThroughHacks.ataPTHacks.returnResponseIgnoreExtendBit)
                {
                    // Extend bit may not be reporting correctly!
                    // Try enabling this and retry this test.
                    ...
                    device->drive_info.passThroughHacks.ataPTHacks.partialRTFRs =
                        false; // Turn this off in case it was set earlier by another test.
                    printf("Received warning the RTFRs are incomplete. Will retry ignoring the extend bit to see if "
                           "this helps.\n");
                    ret = return_Response_Extend_Bit_Test(device);
                    if (ret != SUCCESS)
                    {
            ...
            else
            {
                ...
                set_Console_Colors(true, ERROR_COLOR);
                print_str("ERROR: Command failure while trying to perform RTFR return response info test.\n");
                set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                return FAILURE;
            }
        }
    ...
    else
    {
        ...
        print_str("NOTE: This device doesn't support 48bit commands. Cannot test for proper extend bit in\n");
        print_str("      return response information test. It is strongly recommended that this adapter is retested\n");
        print_str("      with a device that supports 48bit commands so that this can be tested properly\n");
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    }
    return ret;
}
...
{
    uint8_t* log =
        M_REINTERPRET_CAST(uint8_t*, safe_calloc_aligned(logSize, sizeof(uint8_t), device->os_info.minimumAlignment));
    if (log)
    {
        ...
        if (cmdResult == SUCCESS)
        {
            ...
            uint8_t* logR = M_REINTERPRET_CAST(
            ...
            if (logR)
            {
                ...
                if (cmdResult == SUCCESS)
                {
                    if (memcmp(log, logR, logSize) == 0)
                    {
                        ...
                        {
                            set_Console_Colors(true, HACK_COLOR);
                            print_str("HACK FOUND: MMPIO\n");
                            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        }
                        else
                        {
                            print_str("Multi-sector PIO commands work properly!\n");
                        }
                    }
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                print_str("Retrying after changing the multiple mode.\n");
                                if (SUCCESS ==
                                    ata_Set_Multiple_Mode(device, M_Byte0(device->drive_info.IdentifyData.ata.Word047)))
                                {
                                    device->drive_info.passThroughHacks.ataPTHacks.multiSectorPIOWithMultipleMode =
                                        true;
                                ...
                                {
                                    print_str("Unable to set the multiple mode. Aborting multi-sector PIO test\n");
                                    device->drive_info.passThroughHacks.ataPTHacks.multiSectorPIOWithMultipleMode =
                                        false;
                                }
                            }
                            ...
                            {
                                device->drive_info.passThroughHacks.ataPTHacks.multiSectorPIOWithMultipleMode = false;
                                print_str("Unable to perform test of multi-sector PIO commands\n");
                            }
                            else
                            {
                                set_Console_Colors(true, HACK_COLOR);
                ...
                else
                {
                    ...
                    print_str("Unable to read back the log that was written!\n");
            ...
            {
                set_Console_Colors(true, ERROR_COLOR);
                print_str("FATAL ERROR: Unable to allocate enough memory to finish multi-sector PIO test!\n");
                set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
            }
...
{
    print_str("Checking multi-sector PIO command support\n");
    ...
    if (device->drive_info.ata_Options.generalPurposeLoggingSupported)
    {
        if (SUCCESS == ata_Read_Log_Ext(device, ATA_LOG_DIRECTORY, 0, logDir, 512, false, 0))
        {
            ...
            for (uint16_t iter = ATA_LOG_HOST_SPECIFIC_80H * 2; iter < 512 && iter <= ATA_LOG_HOST_SPECIFIC_9FH * 2;
                 iter += 2)
            {
                ...
                if (logSize > 0)
                {
                    ...
                    uint8_t* log = M_REINTERPRET_CAST(uint8_t*, safe_calloc_aligned(allocedLogSize, sizeof(uint8_t),
                        ...
                        else
                        {
                                ...
                                M_Byte0(device->drive_info.IdentifyData.ata.Word047) > 0)
                            {
                                    ...
                                    currentDRQblocks == drqBlocksToSet)
                                {
                                    ...
                                    // already been tested for now. Instead, try changing this to a value of 1 to see if
                                    // changing this to a different value works around the failure seen so far.
                                    drqBlocksToSet = 1;
                                }
                                print_str("Retrying after changing the multiple mode.\n");
                                ...
                                {
                                    print_str("Unable to set the multiple mode. Aborting multi-sector PIO test\n");
                                    // restore the multiple mode back to what the adapter originally configured to be
                                    // safe that it's in a good state.
    ...
    else if (smartSupported && smartLoggingSupported)
    {
        if (SUCCESS == ata_SMART_Read_Log(device, ATA_LOG_DIRECTORY, logDir, 512))
        {
            ...
            //     If the behavior changed and worked, then we know multisector PIO is possible up to the maximum
            //     multiple mode configuration
            for (uint16_t iter = UINT16_C(0x80) * UINT16_C(2);
                 iter < UINT16_C(512) && iter <= UINT16_C(0x9F) * UINT16_C(2); iter += UINT16_C(2))
            {
                uint16_t logSize = M_BytesTo2ByteValue(logDir[iter + 1], logDir[iter]);
                logAddress       = C_CAST(uint8_t, iter / UINT16_C(2));
                if (logSize > 0)
                {
                    uint32_t allocedLogSize = C_CAST(uint32_t, logSize) * UINT32_C(512);
                    uint8_t* log = M_REINTERPRET_CAST(uint8_t*, safe_calloc_aligned(allocedLogSize, sizeof(uint8_t),
    ...
    {
        // If these aren't available or there aren't any multi-sector logs, then will need to use read and write PIO
...
static void sat_DMA_UDMA_Protocol_Test(tDevice*           device,
                                       M_ATTR_UNUSED bool smartSupported,
                                       M_ATTR_UNUSED bool smartLoggingSupported)
{
    // Attempt UDMA mode first
    uint64_t lba      = UINT64_C(0);
    uint16_t sectors  = UINT16_C(1);
    ...
    if (ptrData)
    {
        ...
        {
            set_Console_Colors(true, HACK_COLOR);
            print_str("HACK FOUND: NDMA\n"); // Cannot issue any DMA mode commands.
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
            set_Console_Colors(true, ERROR_COLOR);
            print_str("ERROR: No possible way to issue a DMA mode command on this device.\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        ...
        else
        {
            ...
            {
                set_Console_Colors(true, NOTE_COLOR);
                printf("NOTE: Device only supports UDMA mode for passthrough. This is the default for the tool, but "
            ...
            {
                print_str("Both UDMA and DMA protocols are supported.\n");
            }
            else
            {
                set_Console_Colors(true, ERROR_COLOR);
                print_str("FATAL ERROR: You shouldn't be seeing this message. Something went horribly wrong\n");
                set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
            }
        }
    ...
    {
        set_Console_Colors(true, ERROR_COLOR);
        print_str("ERROR: Unable to allocate memory for DMA/UDMA protocol test\n");
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    }
}
...
static void check_Condition_Bit_Test(tDevice* device, bool smartSupported, bool smartLoggingSupported)
{
    ...
    print_str("Testing for check condition bit support on any command\n");
    ...
    if ((SUCCESS == satRet || WARN_INVALID_CHECKSUM == satRet) && !is_Empty(identifyData, 512))
    {
        ...
        // Try other things like read log (single page) and read log (multiple pages)...need to try a multipage log from
        // GPL or SMART. Doesn't matter which one.
        if (device->drive_info.ata_Options.generalPurposeLoggingSupported)
        {
            // GPL
            // try reading the GPL directory, then read the first multi-page log available to figure out if multiple
            // pages are OK with the check condition bit
            testedGPL = true;
        }
        if (smartSupported)
        {
            ...
            if (smartData)
            {
                ...
                if (SUCCESS == smartRet || WARN_INVALID_CHECKSUM == smartRet)
                {
                    ...
                    if (device->drive_info.lastCommandRTFRs.status != 0 &&
                        !(ATA_STATUS_BIT_ERROR & device->drive_info.lastCommandRTFRs.status) &&
        ...
        {
            printf("WARNING: Neither GPL or SMART were available to test. Please retest with a drive that supports at "
                   "least\n");
...
static void return_Response_Info_Test(tDevice*           device,
                                      M_ATTR_UNUSED bool smartSupported,
                                      M_ATTR_UNUSED bool smartLoggingSupported,
                                      bool               testWithoutTDirAllowed)
{
    print_str("Testing for support of Return Response Info protocol\n");
    // Test return response information - TODO: May need to try issuing some command before we do this test.
    device->drive_info.passThroughHacks.ataPTHacks.returnResponseInfoNeedsTDIR =
        true; // Testing with this on FIRST because it is likely more compatible.
    if (SUCCESS == request_Return_TFRs_From_Device(device, &device->drive_info.lastCommandRTFRs))
    {
        ...
        set_Console_Colors(true, HACK_COLOR);
        print_str("HACK FOUND: RS\n");   // returnResponseInfoSupported
        print_str("HACK FOUND: RSTD\n"); // returnResponseInfoNeedsTDIR
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    }
    else if (testWithoutTDirAllowed)
    {
        device->drive_info.passThroughHacks.ataPTHacks.returnResponseInfoNeedsTDIR = false;
        // TODO: send test unit ready to clear any errors..this may need to loop and retry up to 30 seconds for watchdog
        // timers to kick in and reset the device.
        if (SUCCESS == request_Return_TFRs_From_Device(device, &device->drive_info.lastCommandRTFRs))
        {
            device->drive_info.passThroughHacks.ataPTHacks.returnResponseInfoSupported = true;
            set_Console_Colors(true, HACK_COLOR);
...
typedef struct s_rawDesignator
{
    bool     valid;
...
typedef struct s_scsiDevInfo
{
    ...
    struct
    {
        ...
        uint8_t scsiVersion;
        uint8_t responseFormat; // inq response format. Can help understand if above information came from somewhere
                                // else or not
    ...
    struct
    {
        ...
        uint16_t vpdDSTTimeMinutes; // ext dst from VPD page. If zero, either not reported on this page, or device just
                                    // doesn't do this translation
        ...
        bool unmapSupported;
        // TODO: based on reported VPD pages (mode page policy and/or feature sets, etc), create a list of mode pages
        // that are expected to be supported for the mode page test.
    } vpdData;
    // Log Page data
    struct
    {
        ...
        struct
        {
            uint32_t totalSelfTestEntries; // Only doing this because it is more complicated to compare the fields with
                                           // ATA and is not really worth it right now - TJE
        } selfTestData;
    ...
    struct
    {
        ...
        struct
        {
            bool awre; // true for everything today. Possibly false if it is an old drive....can tell by checking
                       // defaults instead of current values.
            bool arre; // false for ATA, true for NVMe
            uint16_t
                recoveryTimeLimit; // 0 for all ATA devices according to SAT. NVMe will set this to value of a feature.
        } rwErrRecData;
        bool gotCachingPage;
        struct
        {
...
static void scsi_VPD_Pages(tDevice* device, ptrScsiDevInformation scsiDevInfo)
{
    ...
    if (SUCCESS != scsi_Inquiry(device, supportedPages, SUPPORTED_PAGES_LEN, SUPPORTED_VPD_PAGES, true, false))
    {
        dummiedPages = true;
        set_Console_Colors(true, WARNING_COLOR);
        print_str("WARNING: Device did not report supported VPD pages.\n");
        print_str("         VPD pages communicate other device capabilities\n");
        ...
        // if (version >= 3)//SPC
        {
            if (scsiDevInfo->inquiryData.peripheralDeviceType == PERIPHERAL_DIRECT_ACCESS_BLOCK_DEVICE ||
                scsiDevInfo->inquiryData.peripheralDeviceType == PERIPHERAL_SIMPLIFIED_DIRECT_ACCESS_DEVICE ||
                scsiDevInfo->inquiryData.peripheralDeviceType == PERIPHERAL_HOST_MANAGED_ZONED_BLOCK_DEVICE)
            {
    ...
    // definitely strong warning if SAT VPD page is missing when it was expected. This warning should be emitted later
    // if we do SAT passthrough testing as well...try to trust what the device does report If supported VPD pages is
    // missing, attempt to read unit serial number and device identification and block limits pages anyways...report
    // warnings if ANY of these work, but supported pages was missing.
    ...
    {
        set_Console_Colors(true, WARNING_COLOR);
        print_str("WARNING: Supported VPD pages length seems suspiciously large!\n");
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    }
    uint16_t pagesread = UINT16_C(0);
    // TODO: validate peripheral qualifier and peripheral device type on every page with std inquiry data
    for (uint32_t vpdIter = UINT32_C(4);
         vpdIter < C_CAST(uint32_t, (supportedVPDPagesLength + 4)) && vpdIter < INQ_RETURN_DATA_LENGTH; vpdIter++)
    {
        ...
        if (SUCCESS == scsi_Inquiry(device, pageToRead, 4, supportedPages[vpdIter], true, false))
        {
            ...
            if (temp)
            {
                pageToRead = temp;
                if (SUCCESS ==
                    scsi_Inquiry(device, pageToRead, vpdPageLength + 4, supportedPages[vpdIter], true, false))
                {
        ...
        else if (!dummiedPages)
        {
            ...
            printf("ERROR: Failed to read page %" PRIX8 "h. Was reported as supported, but cannot be read.\n",
                   supportedPages[vpdIter]);
            print_str("       Attempted to read only first 4 bytes to determine full VPD page size as spec allows.\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        }
        switch (supportedPages[vpdIter])
        {
        case SUPPORTED_VPD_PAGES:
            print_str("Supported VPD Pages\n");
            break;
        case UNIT_SERIAL_NUMBER:
            print_str("Unit Serial Number VPD Page\n");
            if (readVPDPage)
            {
                if (dummiedPages)
                {
                    set_Console_Colors(true, HACK_COLOR);
                    print_str("HACK FOUND: UNA\n");
                    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                    device->drive_info.passThroughHacks.scsiHacks.unitSNAvailable = true;
                }
                char* unitSerialNumber = M_REINTERPRET_CAST(
                    char*, safe_calloc(vpdPageLength + 1, sizeof(char))); // add 1 for M_NULLPTR terminator
                if (unitSerialNumber)
                {
                    ...
                    for (uint16_t iter = UINT16_C(0); iter < vpdPageLength && iter < UINT16_MAX; ++iter)
                    {
                        if (!safe_isascii(unitSerialNumber[iter]) || !safe_isprint(unitSerialNumber[iter]))
                        {
                            ...
                            set_Console_Colors(true, WARNING_COLOR);
                            print_str("WARNING: Unit Serial Number contains non-ASCII or non-Printable Characters!\n");
                            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        }
                    }
                    ...
                    {
                        set_Console_Colors(true, WARNING_COLOR);
                        print_str("WARNING: Unit Serial Number is empty!\n");
                        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                    }
                    else
                    {
        ...
        case IMPLEMENTED_OPERATING_DEFINITIONS:
            print_str("Implemented Operating Definitions VPD Page\n");
            break;
        case ASCII_IMPLEMENTED_OPERATING_DEFINITION:
            print_str("ASCII Implemented Operation Definition VPD Page\n");
            break;
        case DEVICE_IDENTIFICATION:
            print_str("Device Identification VPD Page\n");
            if (readVPDPage)
            {
                print_str("\tReported Designators:\n");
                genericVPDPageReadOutput  = false;
                uint16_t counter          = UINT16_C(0);
                uint16_t designatorLength = UINT16_C(4); // header length and will be update with each one we look at.
                bool     gotEUI64 = false, gotNAA = false, gotSCSINameString = false, gotUUID = false, gotMD5 = false;
                uint16_t md5Offset                      = UINT16_C(0);
                scsiDevInfo->vpdData.gotDeviceIDVPDPage = true;

                for (uint32_t offset = UINT32_C(4); offset < C_CAST(uint32_t, vpdPageLength + 4) && offset < UINT16_MAX;
                     offset += designatorLength + 4, ++counter)
                {
                    ...
                    case 0: // associated with addressed logical unit
                        print_str("\t\t\tAssociated with the addressed logical unit\n");
                        break;
                    case 1: // associated with the target port that received the command
                            // check PIV & protocol identifier
                        print_str("\t\t\tAssociated with the target port that received the command\n");
                        if (piv)
                        {
                            switch (protocolIdentifier)
                            {
                            case 0:
                                print_str("\t\t\t    Fibre Channel Protocol for SCSI\n");
                                break;
                            case 1:
                                print_str("\t\t\t    Parallel SCSI\n");
                                break;
                            case 2:
                                print_str("\t\t\t    Serial Storage Architecture SCSI-3 Protocol\n");
                                break;
                            case 3:
                                print_str("\t\t\t    Serial Bus Protocol for IEEE 1394\n");
                                break;
                            case 4:
                                print_str("\t\t\t    SCSI RDMA Protocol\n");
                                break;
                            case 5:
                                print_str("\t\t\t    Internet SCSI (iSCSI)\n");
                                break;
                            case 6:
                                print_str("\t\t\t    SAS Serial SCSI Protocol\n");
                                break;
                            case 7:
                                print_str("\t\t\t    Automation/Drive Interface Transport Protocol\n");
                                break;
                            case 8:
                                print_str("\t\t\t    AT Attachment Interface\n");
                                break;
                            case 9:
                                print_str("\t\t\t    USB Attached SCSI\n");
                                break;
                            case 10:
                                print_str("\t\t\t    SCSI over PCI Express\n");
                                break;
                            case 0x0f:
                                print_str("\t\t\t    No specific Protocol\n");
                                break;
                            default:
                    ...
                    case 2: // associated with the SCSI target device that contains the addressed logical unit
                            // check PIV & protocol identifier
                        printf(
                            "\t\t\tAssociated with the SCSI target device that contains the addressed logical unit\n");
                        if (piv)
                        {
                            switch (protocolIdentifier)
                            {
                    ...
                    case 3: // reserved
                        set_Console_Colors(true, WARNING_COLOR);
                        print_str("\t\t\tWARNING: Reserved association\n");
                        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        break;
                    }
                    ...
                    case 0: // vendor specific
                        print_str("\t\t\tVendor Specific Designator:\n");
                        print_Data_Buffer(&pageToRead[offset], designatorLength, true);
                        break;
                    case 1: // T10 vendor ID based
                        print_str("\t\t\tT10 Vendor ID Based Designator:\n");
                        if (!isASCII)
                        {
                            set_Console_Colors(true, WARNING_COLOR);
                            print_str("WARNING: Code set does not mark this as ASCII! This should be ASCII data!!!\n");
                            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        }
                        {
                            ...
                            {
                                set_Console_Colors(true, WARNING_COLOR);
                                printf("WARNING: T10 Vendor ID based designator is missing vendor specific "
                                       "identifier!!!\n");
                                printf("Recommended method from SPC is to concatenate Product ID and Product Serial "
                                       "number\n");
                                set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                            }
                        }
                    ...
                    case 2: // EUI-64 based
                        ...
                        print_str("\t\t\tEUI-64 Designator:\n");
                        // 3 different formats depending on designator length
                        switch (designatorLength)
                        {
                        case 0x08: // EUI64
                            print_str("\t\t\t    EUI64\n");
                            print_str("\t\t\t    IEEE Company ID: ");
                            for (uint32_t ieeeVIDOffset = designatorOffset;
                                 ieeeVIDOffset < C_CAST(uint32_t, (designatorOffset + 3)); ++ieeeVIDOffset)
                            {
                                printf("%02" PRIX8, pageToRead[ieeeVIDOffset]);
                            }
                            print_str("\n");
                            print_str("\t\t\t    Vendor Specific Extension Identifier: ");
                            for (uint32_t vuExtIDOffset = designatorOffset + 3;
                                 vuExtIDOffset < C_CAST(uint32_t, (designatorOffset + 8)); ++vuExtIDOffset)
                            {
                        ...
                        case 0x0C: // EUI64 - 12 byte
                            ...
                            print_str("\n");
                            print_str("\t\t\t    Directory ID: ");
                            for (uint32_t dIDOffset = designatorOffset + 8;
                                 dIDOffset < C_CAST(uint32_t, (designatorOffset + 12)); ++dIDOffset)
                            {
                        ...
                        case 0x10: // EUI64 - 16 byte
                            print_str("\t\t\t    EUI64 - 16 Byte\n");
                            print_str("\t\t\t    Identifier Extension: ");
                            for (uint32_t idExtOffset = designatorOffset;
                                 idExtOffset < C_CAST(uint32_t, (designatorOffset + 8)); ++idExtOffset)
                            {
                                printf("%02" PRIX8, pageToRead[idExtOffset]);
                            }
                            print_str("\n");
                            print_str("\t\t\t    IEEE Company ID: ");
                            for (uint32_t ieeeVIDOffset = designatorOffset + 8;
                                 ieeeVIDOffset < C_CAST(uint32_t, (designatorOffset + 11)); ++ieeeVIDOffset)
                            {
                                printf("%02" PRIX8, pageToRead[ieeeVIDOffset]);
                            }
                            print_str("\n");
                            print_str("\t\t\t    Vendor Specific Extension Identifier: ");
                            for (uint32_t vuExtIDOffset = designatorOffset + 11;
                                 vuExtIDOffset < C_CAST(uint32_t, (designatorOffset + 16)); ++vuExtIDOffset)
                            {
                                printf("%02" PRIX8, pageToRead[vuExtIDOffset]);
                            }
                            print_str("\n");
                        ...
                        default: // unknown EUI64 format!
                            print_str("\t\t\t    Unknown EUI64 designator length!\n");
                            print_str("\t\t\t    Unknown data format: ");
                            // print it out in hex
                            for (uint32_t euiOffset = designatorOffset;
                                 euiOffset < C_CAST(uint32_t, (designatorOffset + designatorLength)); ++euiOffset)
                            {
                    ...
                    case 3: // NAA
                        ...
                        print_str("\t\t\tNetwork Address Authority Designator:\n");
                        // NAA value will be either 2, 4, 5, or 6 which changes length and format
                        {
                            ...
                            case 2: // IEEE Extended - binary only
                                print_str("\t\t\t    NAA Type 2 - IEEE Extended\n");
                                ...
                                printf("\t\t\t        IEEE Company ID: %02" PRIX8 "%02" PRIX8 "%02" PRIX8 "\n",
                                       pageToRead[designatorOffset + 2], pageToRead[designatorOffset + 3],
                                       pageToRead[designatorOffset + 4]);
                                // vendor specific Identifier B
                                printf("\t\t\t        Vendor Specific ID B: %02" PRIX8 "%02" PRIX8 "%02" PRIX8 "\n",
                                       pageToRead[designatorOffset + 5], pageToRead[designatorOffset + 6],
                                       pageToRead[designatorOffset + 7]);
                                break;
                            case 3: // Locally Assigned - binary only
                                print_str("\t\t\t    NAA Type 3 - Locally Assigned\n");
                                // Locally administered value
                                printf("\t\t\t        Locally Administered Value: %01" PRIX8,
                                       M_Nibble0(pageToRead[designatorOffset])); // print the first nibble, the rest is
                                                                                 // all individual bytes
                                for (uint16_t lavOffset = designatorOffset + 1; lavOffset < (designatorLength + 4);
                                     ++lavOffset)
                                {
                            ...
                            case 5: // IEEE Registered - binary only
                                print_str("\t\t\t    NAA Type 5 - IEEE Registered\n");
                                // IEEE Company ID
                                printf("\t\t\t        IEEE Company ID: %01" PRIX8 "%02" PRIX8 "%02" PRIX8 "%01" PRIX8
                                       "\n",
                                       M_Nibble0(pageToRead[designatorOffset]), pageToRead[designatorOffset + 1],
                                       pageToRead[designatorOffset + 2], M_Nibble1(pageToRead[designatorOffset + 3]));
                                // Vendor Specific Identifier
                                printf("\t\t\t        Vendor Specific ID: %01" PRIX8 "%02" PRIX8 "%02" PRIX8 "%02" PRIX8
                                       "%02" PRIX8 "\n",
                                       M_Nibble0(pageToRead[designatorOffset + 3]), pageToRead[designatorOffset + 4],
                                       pageToRead[designatorOffset + 5], pageToRead[designatorOffset + 6],
                                       pageToRead[designatorOffset + 7]);
                                break;
                            case 6: // IEEE Registered Extended - binary only
                                print_str("\t\t\t    NAA Type 6 - IEEE Registered Extended\n");
                                // IEEE Company ID
                                printf("\t\t\t        IEEE Company ID: %01" PRIX8 "%02" PRIX8 "%02" PRIX8 "%01" PRIX8
                                       "\n",
                                       M_Nibble0(pageToRead[designatorOffset]), pageToRead[designatorOffset + 1],
                                ...
                                printf("\t\t\t        Vendor Specific ID Extension: %02" PRIX8 "%02" PRIX8 "%02" PRIX8
                                       "%02" PRIX8 "%02" PRIX8 "%02" PRIX8 "%02" PRIX8 "%02" PRIX8 "\n",
                                       pageToRead[designatorOffset + 8], pageToRead[designatorOffset + 9],
                                       pageToRead[designatorOffset + 10], pageToRead[designatorOffset + 11],
                                       pageToRead[designatorOffset + 12], pageToRead[designatorOffset + 13],
                                       pageToRead[designatorOffset + 14], pageToRead[designatorOffset + 15]);
                                break;
                            default: // unknown NAA Type
                                printf("\t\t\t    Unknown NAA Type %" PRIX8 "h\n", naa);
                                print_str("\t\t\t        Value: ");
                                for (uint16_t unaaOffset = designatorOffset; unaaOffset < (designatorLength + 4);
                                     ++unaaOffset)
                                {
                                    if (codeSet == 2 || codeSet == 3)
                                    {
                                        // ASCII or UTF8...this should be OK, but we could get a unprintable UTF8
                                        // character this way!
                    ...
                    case 4: // relative target port identifier
                        print_str("\t\t\tRelative Target Port Identifier Designator:\n");
                        if (isASCII || isUTF8)
                        {
                            set_Console_Colors(true, WARNING_COLOR);
                            print_str("WARNING: Code set is not set properly! This should be set to 1 for binary!\n");
                            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        }
                        else if (association != 1)
                        {
                            set_Console_Colors(true, WARNING_COLOR);
                            print_str("WARNING: Use of this designator with association not set to 1 is reserved!\n");
                            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        }
                        {
                            uint16_t relativeTargetPortID =
                                M_BytesTo2ByteValue(pageToRead[designatorOffset + 2], pageToRead[designatorOffset + 3]);
                            printf("\t\t\t    Relative Target Port ID: %" PRIu16 "\n", relativeTargetPortID);
                        }
                        break;
                    case 5: // target port group
                        print_str("\t\t\tTarget Port Group Designator:\n");
                        if (isASCII || isUTF8)
                        {
                            set_Console_Colors(true, WARNING_COLOR);
                            print_str("WARNING: Code set is not set properly! This should be set to 1 for binary!\n");
                        ...
                        {
                            uint16_t targetPortGroup =
                                M_BytesTo2ByteValue(pageToRead[designatorOffset + 2], pageToRead[designatorOffset + 3]);
                            printf("\t\t\t    Target Port Group: %" PRIu16 "\n", targetPortGroup);
                        }
                        break;
                    case 6: // logical unit group
                        print_str("\t\t\tLogical Unit Group Designator:\n");
                        if (isASCII || isUTF8)
                        {
                            set_Console_Colors(true, WARNING_COLOR);
                            print_str("WARNING: Code set is not set properly! This should be set to 1 for binary!\n");
                        ...
                        {
                            set_Console_Colors(true, WARNING_COLOR);
                            print_str("WARNING: Use of this designator with association not set to 0 is reserved!\n");
                            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        }
                        {
                            uint16_t luGroup =
                    ...
                    case 7: // MD5 logical unit identifier
                        gotMD5 = true;
                        // This should basically never be used at this point as all the other unique identifiers should
                        // be used instead. This should be handled AFTER checking all other designators in order to warn
                        // if it is in use while other unique designators are present as SPC defines.
                        md5Offset = designatorOffset;
                        break;
                    case 8: // SCSI name string
                        gotSCSINameString = true;
                        print_str("\t\t\tSCSI Name String Designator:\n");
                        if (!isUTF8)
                        {
                            set_Console_Colors(true, WARNING_COLOR);
                            print_str("WARNING: Code set is not properly set! This should be set to 3 for UTF8!\n");
                            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        }
                        {
                    ...
                    case 9: // protocol specific port identifier
                        print_str("\t\t\tProtocol Specific Port Identifier Designator:\n");
                        // this is either USB or PCIe information depending on the protocol identifier field
                        if (association != 0x01)
                        {
                            set_Console_Colors(true, WARNING_COLOR);
                            print_str("WARNING: Association is not set to 1! This may cause parsing issues!\n");
                            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        }
                        if (!piv)
                        {
                            ...
                            set_Console_Colors(true, ERROR_COLOR);
                            printf("ERROR: Protocol identifier valid bit is not set! Cannot parse this without this "
                                   "bit!\n");
                            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        }
                        else
                        {
                            ...
                            case 9: // UAS
                            {
                                ...
                                print_str("\t\t\t    UAS Target Port Identifier:\n");
                            ...
                            {
                                uint16_t routingID =
                                    M_BytesTo2ByteValue(pageToRead[designatorOffset], pageToRead[designatorOffset + 1]);
                                print_str("\t\t\t    PCIe Routing Identifier:\n");
                                printf("\t\t\t        Routing ID: %04" PRIX16 "h\n", routingID);
                            }
                            break;
                            default: // unknown or not defined in SPC5 when this was written
                                print_str("\t\t\t    Unknown Protocol Specific Port Identifier type!\n");
                    ...
                    case 0xA: // UUID
                        ...
                        print_str("\t\t\tUUID Designator:\n");
                        {
                            ...
                            case 1: // locally assigned RFC 4122 UUID
                                ...
                                for (uint16_t uuidOffset = designatorOffset; uuidOffset < (designatorLength + 4);
                                     ++uuidOffset)
                                {
                ...
                {
                    set_Console_Colors(true, WARNING_COLOR);
                    print_str("WARNING: Device reported MD5 designator while also reporting a\n");
                    print_str("         unique identifier as well. This is not allowed per SPC!\n");
                    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                }
                else if (gotMD5 && md5Offset)
                {
                    // TODO: Validate the MD5 output by computing it ourselves from other designators that were
                    // reported! NOTE: Any designators not available are replaced with 8 space characters per spec as
                    // input to the MD5 computation
                    print_str("\t\t\tMD5 Logical Unit Designator:\n");
                    print_str("\t\t\t    MD5 Logical Unit Identifier: ");
        ...
        case SOFTWARE_INTERFACE_IDENTIFICATION:
            print_str("Software Interface Identification VPD Page\n");
            break;
        case MANAGEMENT_NETWORK_ADDRESSES:
            print_str("Management Network Addresses VPD Page\n");
            break;
        case EXTENDED_INQUIRY_DATA: // While this page is useful, it is only in SAT4 and only and only if read buffer
                                    // command is supported. SAT5 adds in support for download microcode modes supported
            print_str("Extended Inquiry Data VPD Page\n");
            ...
            if (readVPDPage)
            {
                ...
                {
                    set_Console_Colors(true, ERROR_COLOR);
                    printf("ERROR: VPD Page length is less than specified in SPC! Expected %" PRIX16
                           "h, but got %" PRIX16 "h\n",
                ...
                else
                {
                    ...
                    // Need to figure out which things are actually useful to show! Will start with things that are most
                    // useful
                    uint16_t extDSTMinutes = M_BytesTo2ByteValue(pageToRead[10], pageToRead[11]);
                    if (extDSTMinutes == 0)
                    {
                        set_Console_Colors(true, WARNING_COLOR);
                        printf("WARNING: Extended DST time in minutes was reported as zero! This means SCSI DST "
        ...
        case MODE_PAGE_POLICY:
            print_str("Mode Page Policy VPD Page\n");
            // TODO: Parse this page to figure out which mode pages are supported by the device so that the mode page
            // checking section can be validated better!
            break;
        case SCSI_PORTS:
            print_str("SCSI Ports VPD Page\n");
            break;
        case ATA_INFORMATION:
            print_str("ATA Information VPD Page\n");
            if (readVPDPage)
            {
                if (vpdPageLength < 0x0238)
                {
                    set_Console_Colors(true, ERROR_COLOR);
                    printf("ERROR: VPD Page length is less than specified in SAT! Expected %" PRIX16
                           "h, but got %" PRIX16 "h\n",
                ...
                else
                {
                    ...
                    for (uint8_t iter = UINT8_C(0); iter < 8; ++iter)
                    {
                        if (!safe_isascii(satVendor[iter]) || !safe_isprint(satVendor[iter]))
                        {
                            satVendor[iter] = ' ';
                            set_Console_Colors(true, WARNING_COLOR);
                            print_str("WARNING: SAT Vendor ID contains non-ASCII or non-Printable Characters!\n");
                            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        }
                    }
                    ...
                    {
                        set_Console_Colors(true, WARNING_COLOR);
                        print_str("WARNING: SAT Vendor ID is empty!\n");
                        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                    }
                    else
                    {
                    ...
                    for (uint8_t iter = UINT8_C(0); iter < 16; ++iter)
                    {
                        if (!safe_isascii(satProductID[iter]) || !safe_isprint(satProductID[iter]))
                        {
                            satProductID[iter] = ' ';
                            set_Console_Colors(true, WARNING_COLOR);
                            print_str("WARNING: SAT Product ID contains non-ASCII or non-Printable Characters!\n");
                            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        }
                    }
                    ...
                    {
                        set_Console_Colors(true, WARNING_COLOR);
                        print_str("WARNING: SAT Product ID is empty!\n");
                        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                    }
                    else
                    {
                    ...
                    for (uint8_t iter = UINT8_C(0); iter < 4; ++iter)
                    {
                    ...
                    {
                        set_Console_Colors(true, WARNING_COLOR);
                        print_str("WARNING: SAT Product Revision is empty!\n");
                        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                    }
                    else
                    {
                    ...
                    uint16_t signatureOffset = UINT16_C(36);
                    bool     isSATA          = false;
                    print_str("\tATA Device Signature:\n");
                    switch (pageToRead[signatureOffset])
                    {
                    case 0x00: // PATA
                        print_str("\t\tTransport: PATA\n");
                        break;
                    case 0x34: // SATA
                        print_str("\t\tTransport: SATA\n");
                        isSATA = true;
                        break;
                    default: // unknown
                    ...
                    // command code
                    print_str("\tIdentify Command Code: ");
                    switch (pageToRead[56])
                    {
                    case 0xEC: // Identify
                        print_str("Identify (ECh)\n");
                        break;
                    case 0xA1: // Identify Packet
                        print_str("Identify Packet (A1h)\n");
                        break;
                    case 0x2F: // Read Log Ext
                        print_str("Read Log Ext (2Fh)\n");
                        break;
                    case 0x47: // Read Log Ext DMA
                        print_str("Read Log Ext DMA (47h)\n");
                        break;
                    default: // unknown
                        printf("Unknown (%02" PRIX8 "h)\n", pageToRead[56]);
                        break;
                    }
                    print_str("\tIdentify Data:\n");
                    print_Data_Buffer(&pageToRead[60], 512, true);
                }
            }
            break;
        case POWER_CONDITION:
            print_str("Power Condition VPD Page\n");
            ...
            if (readVPDPage)
            {
                ...
                else
                {
                    ...
                    {
                        print_str("\tStopped Condition Recovery Time: >65.534 milliseconds\n");
                    }
                    else
                    {
                    ...
                    {
                        print_str("\tStandby Z Supported\n");
                        print_str("\t    Recovery Time: ");
        ...
        case DEVICE_CONSTITUENTS:
            print_str("Device Constituents VPD Page\n");
            break;
        case CFA_PROFILE_INFORMATION:
            print_str("CFA Profile Information VPD Page\n");
            break;
        case POWER_CONSUMPTION:
            print_str("Power Consumption VPD Page\n");
            break;
        case THIRD_PARTY_COPY:
            print_str("Third Party Copy VPD Page\n");
            break;
        case PROTOCOL_SPECIFIC_LU_INFO:
            print_str("Protocol Specific Logical Unit Information VPD Page\n");
            break;
        case PROTOCOL_SPECIFIC_PORT_INFO:
            print_str("Protocol Specific Port Information VPD Page\n");
            break;
        case SCSI_FEATURE_SETS:
            print_str("SCSI Feature Sets VPD Page\n");
            if (readVPDPage)
            {
                genericVPDPageReadOutput = false;
                for (uint32_t featIter = UINT32_C(8);
                     featIter < C_CAST(uint32_t, (vpdPageLength + 4)) && featIter < UINT16_MAX; featIter += 2)
                {
                    uint16_t featureSetCode = M_BytesTo2ByteValue(pageToRead[featIter], pageToRead[featIter + 1]);
                    switch (featureSetCode)
                    {
                    case 0x0001:
                        print_str("\tSPC Discovery 2016 Feature Set\n");
                        print_str("\t\tMandatory Commands:\n");
                        print_str("\t\t\tInquiry\n");
                        print_str("\t\t\tMode Sense 10\n");
                        print_str("\t\t\tReport Luns\n");
                        print_str("\t\t\tReport Supported Operation Codes\n");
                        print_str("\t\t\tTest Unit Ready\n");
                        print_str("\t\tMandatory Mode Pages:\n");
                        print_str("\t\t\tControl\n");
                        print_str("\t\t\tControl Extension\n");
                        print_str("\t\tMandatory VPD Pages:\n");
                        print_str("\t\t\tDevice Identification\n");
                        print_str("\t\t\tExtended Inquiry Data\n");
                        print_str("\t\t\tMode Page Policy\n");
                        print_str("\t\t\tSCSI Feature Sets\n");
                        print_str("\t\t\tSupported VPD Pages\n");
                        break;
                    case 0x0102: // SBC Base 2010
                        print_str("\tSBC Base 2010 Feature Set\n");
                        print_str("\t\tMandatory Commands:\n");
                        print_str("\t\t\tFormat Unit\n");
                        print_str("\t\t\tInquiry\n");
                        print_str("\t\t\tMode Select 10\n");
                        print_str("\t\t\tMode Sense 10\n");
                        print_str("\t\t\tRead 10\n");
                        print_str("\t\t\tRead Capacity 10\n");
                        print_str("\t\t\tReport Luns\n");
                        print_str("\t\t\tRequest Sense\n");
                        print_str("\t\t\tStart Stop Unit\n");
                        print_str("\t\t\tSynchronize Cache 10\n");
                        print_str("\t\t\tTest Unit Ready\n");
                        print_str("\t\t\tWrite 10\n");
                        print_str("\t\t\tWrite Same 10\n");
                        print_str("\t\tMandatory Mode Pages:\n");
                        print_str("\t\t\tCaching\n");
                        print_str("\t\t\tControl\n");
                        print_str("\t\t\tControl Extension\n");
                        print_str("\t\t\tRead-Write Error Recovery\n");
                        print_str("\t\tMandatory VPD Pages:\n");
                        print_str("\t\t\tATA Information (SAT devices only)\n");
                        print_str("\t\t\tBlock Device Characteristics\n");
                        print_str("\t\t\tDevice Identification\n");
                        print_str("\t\t\tExtended Inquiry Data\n");
                        print_str("\t\t\tMode Page Policy\n");
                        print_str("\t\t\tSCSI Feature Sets\n");
                        print_str("\t\t\tSupported VPD Pages\n");
                        print_str("\t\t\tPower Condition\n");
                        break;
                    case 0x0101: // SBC Base 2016
                        print_str("\tSBC Base 2016 Feature Set\n");
                        print_str("\t\tMandatory Commands:\n");
                        print_str("\t\t\tFormat Unit\n");
                        print_str("\t\t\tInquiry\n");
                        print_str("\t\t\tMode Select 10\n");
                        ...
                        print_str("\t\t\tTest Unit Ready\n");
                        print_str("\t\t\tWrite 16\n");
                        print_str("\t\t\tWrite Same 16\n");
                        print_str("\t\tMandatory Mode Pages:\n");
                        print_str("\t\t\tCaching\n");
                        print_str("\t\t\tControl\n");
                        print_str("\t\t\tControl Extension\n");
                        print_str("\t\t\tInformation Exceptions Control\n");
                        print_str("\t\t\tRead-Write Error Recovery\n");
                        print_str("\t\tMandatory VPD Pages:\n");
                        print_str("\t\t\tATA Information (SAT devices only)\n");
                        print_str("\t\t\tBlock Device Characteristics\n");
                        print_str("\t\t\tBlock Limits\n");
                        print_str("\t\t\tDevice Identification\n");
                        print_str("\t\t\tExtended Inquiry Data\n");
                        print_str("\t\t\tMode Page Policy\n");
                        print_str("\t\t\tSCSI Feature Sets\n");
                    ...
                    case 0x0103: // Basic Provisioning 2016
                        print_str("\tBasic Provisioning 2016 Feature Set\n");
                        print_str("\t\tMandatory Commands:\n");
                        print_str("\t\t\tGet LBA Status\n");
                        print_str("\t\t\tRead Capacity 16\n");
                        print_str("\t\t\tUnmap\n");
                        print_str("\t\t\tWrite Same 16\n");
                        print_str("\t\tMandatory VPD Pages:\n");
                    ...
                    case 0x0104: // Drive Maintenance 2016
                        print_str("\tDrive Maintenance 2016 Feature Set\n");
                        print_str("\t\tMandatory Commands:\n");
                        print_str("\t\t\tFormat Unit\n");
                        print_str("\t\t\tLog Select\n");
                        print_str("\t\t\tLog Sense\n");
                        print_str("\t\t\tRead Buffer 10\n");
                        print_str("\t\t\tRead Defect Data 12\n");
                        print_str("\t\t\tReasign Blocks\n");
                        print_str("\t\t\tSanitize\n");
                        print_str("\t\t\tSend Diagnostics\n");
                        print_str("\t\t\tReceive Diagnostic Results\n");
                        print_str("\t\t\tWrite Buffer\n");
                        print_str("\t\t\tWrite Long 16\n");
                        print_str("\t\tMandatory VPD Pages:\n");
                        print_str("\t\t\tExtended Inquiry Data\n");
                        print_str("\t\t\tBlock Device Characteristics\n");
                        print_str("\t\t\tPower Consumption\n");
                        print_str("\t\tMandatory Log Pages:\n");
                        print_str("\t\t\tSupported Pages\n");
                        print_str("\t\t\tSupported Pages and Subpages\n");
                        print_str("\t\t\tBackground Scan\n");
                        print_str("\t\t\tInformational Exceptions\n");
                        print_str("\t\t\tNon-Medium Error\n");
                        print_str("\t\t\tNon-volatile Cache\n");
                        print_str("\t\t\tRead Error Counters\n");
                        print_str("\t\t\tSelf-Test Results\n");
                        print_str("\t\t\tSolid State Media\n");
                        print_str("\t\t\tStart-Stop Cycle Counter\n");
            ...
            break;
        case BLOCK_LIMITS:
            print_str("Block Limits VPD Page\n");
            if (readVPDPage)
            {
                if (vpdPageLength < 0x003C)
                {
                    set_Console_Colors(true, ERROR_COLOR);
                    printf("ERROR: VPD Page length is less than specified in SBC! Expected %" PRIX16
                           "h, but got %" PRIX16 "h\n",
                           0x003C, vpdPageLength);
                    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                }
                else
                {
                    ...
                    print_str("\tOptimal Transfer Length Granularity: ");
                    ...
                    print_str("\tMaximum Transfer Length: ");
                    ...
                    print_str("\tOptimal Transfer Length: ");
                    ...
                    print_str("\tMaximum Prefetch Length: ");
                    ...
                    {
                        print_str("Not Reported or prefetch not supported\n");
                    }
                    print_str("\tMaximum Unmap LBA Count: ");
                    ...
                    print_str("\tMaximum Unmap Block Descriptor Count: ");
                    ...
                    print_str("\tOptimal Unmap Granularity: ");
                    ...
                    {
                        print_str("\tUnmap Granularity Alignment: ");
                        printf(
                            "%" PRIu32 " Logical Blocks\n",
                            M_BytesTo4ByteValue(pageToRead[32] ^ BIT7, pageToRead[33], pageToRead[34], pageToRead[35]));
                    }
                    print_str("\tMaximum Write Same Length: ");
                    if (M_BytesTo8ByteValue(pageToRead[36], pageToRead[37], pageToRead[38], pageToRead[39],
                                            pageToRead[40], pageToRead[41], pageToRead[42], pageToRead[43]) > 0)
                    {
                        printf("%" PRIu64 " Logical Blocks\n",
                               M_BytesTo8ByteValue(pageToRead[36], pageToRead[37], pageToRead[38], pageToRead[39],
                                                   pageToRead[40], pageToRead[41], pageToRead[42], pageToRead[43]));
                    }
                    else
                    {
                        print_str("Not Reported or write same not supported\n");
                    }
                    if (pageToRead[4] & BIT0)
                    {
                        printf("\tWrite same command does not allow number of logical blocks to be zero (write full "
                               "LBA space the same)\n");
                    }
                    ...
                    {
                        print_str("Not Reported or write atomic not supported\n");
                    }
                    print_str("\tAtomic Alignment: ");
                    ...
                    print_str("\tAtomic Transfer Length Granularity: ");
                    ...
                    print_str("\tMaximum Atomic Transfer Length With Atomic Boundary: ");
                    ...
                    print_str("\tMaximum Atomic Boundary Size: ");
        ...
        case BLOCK_DEVICE_CHARACTERISTICS:
            print_str("Block Device Characteristics VPD Page\n");
            if (readVPDPage)
            {
                if (vpdPageLength < 0x003C)
                {
                ...
                else
                {
                    ...
                    scsiDevInfo->vpdData.blockCharacteristicsData.rotationRate =
                        M_BytesTo2ByteValue(pageToRead[4], pageToRead[5]);
                    print_str("\tMedium Rotation Rate: ");
                    ...
                    {
                        print_str("Non-rotating medium\n");
                    }
                    else if (scsiDevInfo->vpdData.blockCharacteristicsData.rotationRate >= 0x0401 &&
                             scsiDevInfo->vpdData.blockCharacteristicsData.rotationRate <= 0xFFFE)
                    {
                        printf("%" PRIu16 " RPM\n", scsiDevInfo->vpdData.blockCharacteristicsData.rotationRate);
                    }
                    else
                    {
                        set_Console_Colors(true, ERROR_COLOR);
                        print_str("ERROR: Reserved value\n");
                        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                    }
                    print_str("\tProduct Type: ");
                    ...
                    {
                    case 0:
                        print_str("Not indicated\n");
                        break;
                    case 1:
                        print_str("CFast\n");
                        break;
                    case 2:
                        print_str("CompactFlash\n");
                        break;
                    case 3:
                        print_str("Memory Stick\n");
                        break;
                    case 4:
                        print_str("MultiMediaCard\n");
                        break;
                    case 5:
                        print_str("Secure Digital Card\n");
                        break;
                    case 6:
                        print_str("XQD\n");
                        break;
                    case 7:
                        print_str("Universal Flash Storage\n");
                        break;
                    default:
                        if (scsiDevInfo->vpdData.blockCharacteristicsData.productType >= 8 &&
                            scsiDevInfo->vpdData.blockCharacteristicsData.productType <= 0xEF)
                        {
                            print_str("Reserved\n");
                        }
                        else
                    ...
                    case 0:
                        ...
                        break;
                    case 1:
                        print_str("Host Aware Zoned Block Device\n");
                        break;
                    case 2:
                        print_str("Device Managed Zoned Block Device\n");
                        break;
                    default:
                        print_str("Reserved\n");
                        break;
        ...
        case LOGICAL_BLOCK_PROVISIONING:
            print_str("Logical Block Provisioning VPD Page\n");
            if (readVPDPage)
            {
                ...
                else
                {
                    ...
                    print_str("\tUnmap Support in Write Same 16: ");
                    ...
                    print_str("\tUnmap Support in Write Same 10: ");
                    ...
                    {
                        print_str("Provisioning Initialization Pattern\n");
                    }
                    else
                    {
                        print_str("Reserved\n");
                    }
                    print_str("\tAnchor Support: ");
                    ...
                    print_str("\tProvisioning Type: ");
                    switch (provisioningType)
                    {
                    case 0:
                        print_str("Not reported or fully provisioned\n");
                        break;
                    case 1:
                        print_str("Resource Provisioned\n");
                        break;
                    case 2:
                        print_str("Thin Provisioned\n");
                        break;
                    default:
                        print_str("Reserved\n");
                        break;
            ...
            break;
        case REFERALS:
            print_str("Referals VPD Page\n");
            break;
        case SUPPORTED_BLOCK_LENGTHS_AND_PROTECTION_TYPES:
            print_str("Supported Block Lengths And Protection Types VPD Page\n");
            break;
        case BLOCK_DEVICE_CHARACTERISTISCS_EXT:
            print_str("Block Device Characteristics Ext VPD Page\n");
        ...
        case ZONED_BLOCK_DEVICE_CHARACTERISTICS:
            print_str("Zoned Block Device Characteristics VPD Page\n");
            // TODO: Thorough parsing of this page and outputting its information in a nice format
            break;
        case BLOCK_LIMITS_EXTENSION:
            print_str("Block Limits Extension VPD Page\n");
            // TODO: Thorough parsing of this page and outputting its information in a nice format
            break;
        default:
...
static eReturnValues scsi_Information(tDevice* device, ptrScsiDevInformation scsiDevInfo)
{
    set_Console_Colors(true, HEADING_COLOR);
    print_str("\n====================\n");
    print_str("Reading Inquiry Data\n");
    print_str("====================\n");
    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    ...
    if (SUCCESS == scsi_Inquiry(device, (uint8_t*)&device->drive_info.scsiVpdData.inquiryData, 96, 0, false, false))
    {
        ...
        default:
            print_str("Unknown or no device type\n");
            break;
        }
        // check rmb bit (show warning if this is set unless this truly is ejectable media, which an HDD is not)
        if (inqPtr[1] & BIT7)
        {
            print_str("\t\tRMB bit is set meaning the recording medium is removable. Ex: CD/DVD\n");
            scsiDevInfo->inquiryData.rmb = true;
        }

        // Report value for version, translated to SCSI/SPC version. Report warnings if below SPC3 since this is when
        // SAT was first released (version1 in 2006)
        print_str("\t\tSCSI Version: ");
        device->drive_info.scsiVersion = scsiDevInfo->inquiryData.scsiVersion = inqPtr[2];
        switch (inqPtr[2])
        {
        case 0:
            print_str("No version compliance reported. This is typical for very old devices...SASI devices\n");
            device->drive_info.scsiVersion = SCSI_VERSION_NO_STANDARD;
            //              if (device->drive_info.interface_type != USB_INTERFACE)
            //              {
            //                  checkForSAT = false; //NOTE: some cheap USB to SATA/PATA adapters will set this version
            //                  or no version. The only way to work around this, is to make sure the low level for the
            //                  OS detects it on USB interface and it can be run through the usb_hacks file instead.
            //              }
            break;
        case 0x01:
        case 0x81:
        ...
        default:
            ...
            // numbers
            if ((inqPtr[2] >= 0x08 && inqPtr[2] <= 0x0C) || (inqPtr[2] >= 0x40 && inqPtr[2] <= 0x44) ||
                (inqPtr[2] >= 0x48 && inqPtr[2] <= 0x4C) || (inqPtr[2] >= 0x80 && inqPtr[2] <= 0x84) ||
                (inqPtr[2] >= 0x88 && inqPtr[2] <= 0x8C))
            {
                ...
                printf("Treating as - (%" PRIX8
                       "h) for legacy device compatibility since version is likely ANSI+ECMA+ISO format\n",
        ...
        // Check response data format. This should be 2h. If not, then this is a device from before SCSI2 which means
        // nothing else is parsable. If below 2, report a strong warning that the fields below are all vendor unique and
        // may not be parsable. Ask if this product is older than 1993??? CCS was released in 1986....
        ...
        if (scsiDevInfo->inquiryData.responseFormat < 2)
        {
            ...
            print_str("         This means that all product identification is in a vendor specific\n");
            print_str("         format and cannot be parsed correctly!\n");
            print_str("         This is only expected for OLD SCSI devices. Nothing else should\n");
            print_str("         report in this format. Raw output is provided which may be usable\n");
            print_str("         if needed for better legacy device support.\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
            set_Console_Colors(true, HACK_COLOR);
            print_str("HACK FOUND: PRESCSI2\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        ...
        else
        {
            ...
            {
                if (!safe_isascii(scsiDevInfo->inquiryData.vendorId[iter]) ||
                    !safe_isprint(scsiDevInfo->inquiryData.vendorId[iter]))
                {
                    scsiDevInfo->inquiryData.vendorId[iter] = ' ';
                    set_Console_Colors(true, WARNING_COLOR);
                    print_str("WARNING: Vendor ID contains non-ASCII or non-Printable Characters!\n");
                    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                }
            }
            ...
            {
                set_Console_Colors(true, WARNING_COLOR);
                print_str("WARNING: Vendor ID is empty!\n");
                set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
            }
            else
            {
            ...
            {
                if (!safe_isascii(scsiDevInfo->inquiryData.productId[iter]) ||
                    !safe_isprint(scsiDevInfo->inquiryData.productId[iter]))
                {
                    scsiDevInfo->inquiryData.productId[iter] = ' ';
                    set_Console_Colors(true, WARNING_COLOR);
                    print_str("WARNING: Product ID contains non-ASCII or non-Printable Characters!\n");
                    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                }
            }
            ...
            {
                set_Console_Colors(true, WARNING_COLOR);
                print_str("WARNING: Product ID is empty!\n");
                set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
            }
            else
            {
            ...
            {
                if (!safe_isascii(scsiDevInfo->inquiryData.productRev[iter]) ||
                    !safe_isprint(scsiDevInfo->inquiryData.productRev[iter]))
                {
                    scsiDevInfo->inquiryData.productRev[iter] = ' ';
                    set_Console_Colors(true, WARNING_COLOR);
                    print_str("WARNING: Product Revision contains non-ASCII or non-Printable Characters!\n");
                    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                }
            }
            ...
            {
                set_Console_Colors(true, WARNING_COLOR);
                print_str("WARNING: Product Revision is empty!\n");
                set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
            }
            else
            {
            ...
            // TODO: based on what was reported for these, check passthrough data later to see if the vendor/product
            // were just concatenated together instead of following translation NOTE: Many USB products report something
            // completely different from the encapsulated drive to make it appear as a specific product in the system.
            // This is not abnormal, but may be a light warning.
            ...
            if (totalInqLength > 36) // checking if inquire data is longer since it was only 36bytes in SCSI2
            {
                ...
                {
                    print_str("Checking Version Descriptors:\n");
                    if (scsiDevInfo->inquiryData.scsiVersion >= 4)
                    {
                        ...
                        for (; versionIter < 8; versionIter++)
                        {
                            ...
                            scsiDevInfo->inquiryData.versionDescriptors[versionIter] =
                                M_BytesTo2ByteValue(device->drive_info.scsiVpdData.inquiryData[(versionIter * 2) + 58],
                                                    device->drive_info.scsiVpdData.inquiryData[(versionIter * 2) + 59]);
                            ...
                            if (is_Standard_Supported(scsiDevInfo->inquiryData.versionDescriptors[versionIter],
                                                      STANDARD_CODE_SAT) ||
                                is_Standard_Supported(scsiDevInfo->inquiryData.versionDescriptors[versionIter],
                                                      STANDARD_CODE_SAT2) ||
                                is_Standard_Supported(scsiDevInfo->inquiryData.versionDescriptors[versionIter],
                                                      STANDARD_CODE_SAT3) ||
                                is_Standard_Supported(scsiDevInfo->inquiryData.versionDescriptors[versionIter],
                                                      STANDARD_CODE_SAT4)
                                // Next version descriptors aren't sat but should only appear on a SAT interface...at
                                // least we know they are ATA/ATAPI so it won't hurt to try issuing a command to the
                                // drive.
                                || is_Standard_Supported(scsiDevInfo->inquiryData.versionDescriptors[versionIter],
                                                         STANDARD_CODE_ATA_ATAPI6) ||
                                is_Standard_Supported(scsiDevInfo->inquiryData.versionDescriptors[versionIter],
                                                      STANDARD_CODE_ATA_ATAPI7) ||
                                is_Standard_Supported(scsiDevInfo->inquiryData.versionDescriptors[versionIter],
                                                      STANDARD_CODE_ATA_ATAPI8) ||
                                is_Standard_Supported(scsiDevInfo->inquiryData.versionDescriptors[versionIter],
                                                      STANDARD_CODE_ACSx))
                            {
                    ...
                    {
                        printf("\tThis device reports SPC compliance which is prior to vesrion descriptors being "
                               "added.\n");
                    }
                }
            }
            ...
            {
                set_Console_Colors(true, NOTE_COLOR);
                print_str("NOTE: Inquiry data is 36 bytes or less but reports format 2 which should be 96\n");
                print_str("      bytes. In these bytes version descriptors will be reported and can be used\n");
                print_str("      to help better understand device capabilities (starting with SPC2)\n\n");
                set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
            }
        }
    ...
    {
        set_Console_Colors(true, ERROR_COLOR);
        print_str("Fatal Error: Unable to get standard inquiry data. Unable to proceed with any more testing!\n");
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        return FAILURE;
    }
    return SUCCESS;
...
{
    // Read capacity. Start with 10, then do 16. Emit warnings when 16 doesn't work and the scsi versions is greater
    // than SPC2 If read capacity 10 comes back saying UINT32_MAX for capacity, warn that this mismatches the SCSI
    // version reported.
    set_Console_Colors(true, HEADING_COLOR);
    print_str("\n========================================\n");
    print_str("Getting Read Capacity data. 10 & 16 byte\n");
    print_str("========================================\n");
    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    ...
    if (SUCCESS == (readCap10Result = scsi_Read_Capacity_10(device, readCapacityData, 8)))
    {
        scsiDevInfo->readCapData.rc10MaxLBA =
            M_BytesTo4ByteValue(readCapacityData[0], readCapacityData[1], readCapacityData[2], readCapacityData[3]);
        scsiDevInfo->readCapData.rc10blockSize =
            M_BytesTo4ByteValue(readCapacityData[4], readCapacityData[5], readCapacityData[6], readCapacityData[7]);
        print_str("\tRead Capacity 10 data:\n");
    ...
    {
        set_Console_Colors(true, WARNING_COLOR);
        print_str("WARNING: Failed read capacity 10. While this command is superceeded by read capacity 16,\n");
        print_str("         supporting it helps legacy system support and software. If not supported, it should\n");
    ...
    if (SUCCESS == (readCap16Result = scsi_Read_Capacity_16(device, readCapacityData, 32)))
    {
        scsiDevInfo->readCapData.rc16MaxLBA =
            M_BytesTo8ByteValue(readCapacityData[0], readCapacityData[1], readCapacityData[2], readCapacityData[3],
                                readCapacityData[4], readCapacityData[5], readCapacityData[6], readCapacityData[7]);
        scsiDevInfo->readCapData.rc16blockSize =
            M_BytesTo4ByteValue(readCapacityData[8], readCapacityData[9], readCapacityData[10], readCapacityData[11]);
        scsiDevInfo->readCapData.rc16physBlockSize =
            C_CAST(uint32_t, power_Of_Two(M_Nibble0(readCapacityData[13]))) * scsiDevInfo->readCapData.rc16blockSize;
        print_str("\tRead Capacity 16 data:\n");
    ...
    if (readCap16Result == SUCCESS && readCap10Result == SUCCESS)
    {
        ...
        {
            set_Console_Colors(true, ERROR_COLOR);
            print_str("ERROR: Block length does not match between read capacity commands!\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        }
        if (scsiDevInfo->readCapData.rc10MaxLBA != UINT32_MAX &&
            scsiDevInfo->readCapData.rc10MaxLBA != scsiDevInfo->readCapData.rc16MaxLBA)
        {
            set_Console_Colors(true, ERROR_COLOR);
            print_str("ERROR: Max LBA does not match between read capacity commands!\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        }
    }
    return SUCCESS;
...
static eReturnValues use_Mode_Sense_6(tDevice* device, uint8_t pageCode, bool* use6Byte)
{
    ...
    // TODO: This code currently always sets DBD, which is fine for SCSI2+, but not ok for earlier devices as that bit
    ...
    if (SUCCESS != scsi_Mode_Sense_10(device, pageCode, 0, 0, false, false, MPC_CURRENT_VALUES, M_NULLPTR))
    {
        ...
        if (is_Invalid_Opcode(device->drive_info.lastCommandSenseData, SPC3_SENSE_LEN))
        {
            ...
            else
            {
                ...
                {
                    set_Console_Colors(true, ERROR_COLOR);
                    print_str("ERROR: This device does not appear to support either mode sense 10 or mode sense 6!\n");
                    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                    return FAILURE;
                }
                else
        ...
        if (tryAnotherPage)
        {
            ...
            if (pageCode != 0x01)
            {
                return use_Mode_Sense_6(device, 0x01,
                                        use6Byte); // trying page 0x01 since any device type SHOULD have that page. For
                                                   // direct access block, it is the RW error recovery page
            }
            else
            {
...
static eReturnValues get_SCSI_Mode_Page_Data(
    tDevice*  device,
    uint8_t   pageCode,
    uint8_t   subPageCode,
    bool      sixByte,
    uint8_t** dataBuffer /*to allow for reallocation/resize if necessary to get the full page*/,
    uint32_t* dataBufferLength)
{
    ...
    if (sixByte)
    {
        ret = scsi_Mode_Sense_6(device, pageCode, C_CAST(uint8_t, M_Min(*dataBufferLength, UINT8_MAX)), subPageCode,
                                false, MPC_CURRENT_VALUES, *dataBuffer);
        if (ret == SUCCESS)
        {
            ...
            {
                set_Console_Colors(true, ERROR_COLOR);
                printf("ERROR: Incorrect page returned from mode sense 6. Expected %02" PRIX8 "h, but got %02" PRIX8
                       "h\n",
            ...
            {
                set_Console_Colors(true, ERROR_COLOR);
                printf("ERROR: Incorrect subpage returned from mode sense 6. Expected %02" PRIX8 "h, but got %02" PRIX8
                       "h\n",
            ...
            else
            {
                ...
                pageLengthValidation +=
                    C_CAST(uint8_t, pageLength + MODE_PARAMETER_HEADER_6_LEN + blockDescriptorLength);
                if (*dataBufferLength < pageLengthValidation)
                {
                    ...
                    if (temp != M_NULLPTR)
                    {
                        ...
                        *dataBufferLength = pageLengthValidation;
                        return get_SCSI_Mode_Page_Data(device, pageCode, subPageCode, sixByte, dataBuffer,
                                                       dataBufferLength);
                    }
                    else
                    {
    ...
    {
        ret = scsi_Mode_Sense_10(device, pageCode, *dataBufferLength, subPageCode, false, false, MPC_CURRENT_VALUES,
                                 *dataBuffer);
        if (ret == SUCCESS)
        {
            ...
            else
            {
                // Everything looks ok, so double check the datalength
                pageLengthValidation +=
                    C_CAST(uint8_t, pageLength + MODE_PARAMETER_HEADER_10_LEN + blockDescriptorLength);
                if (*dataBufferLength < pageLengthValidation)
                {
                    // reallocate enough data and reread the page.
                    uint8_t* temp =
...
static eReturnValues scsi_Mode_Information(tDevice* device, ptrScsiDevInformation scsiDevInfo)
{
    ...
    if (SUCCESS != sixTest)
    {
        ...
        {
            set_Console_Colors(true, WARNING_COLOR);
            printf("WARNING: This device does not seem to support any standard mode pages. Skipping all mode page "
                   "checks\n");
        ...
        {
            set_Console_Colors(true, HACK_COLOR);
            print_str("HACK FOUND: NMP\n");
            device->drive_info.passThroughHacks.scsiHacks.noModePages = true;
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
            set_Console_Colors(true, ERROR_COLOR);
            printf("ERROR: This device doesn't support ANY mode sense commands. Mode sense commands can convey device "
                   "support and change\n");
            printf("       device settings/capabilties. This can be especially important for disabling write caching "
                   "for backups.\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        }
        return sixTest;
    ...
    uint32_t commonModeDataLength =
        modeHeaderLength + 4 +
        8; // Header length + 4 bytes to check initial size of a mode page + 8 bytes for typical short block descriptor.
    uint32_t modeDataLength = UINT32_C(0);
    ...
    if (SUCCESS == get_SCSI_Mode_Page_Data(device, MP_CONTROL, 0, use6Byte, &modeData, &modeDataLength))
    {
        ...
        if (SUCCESS == get_SCSI_Mode_Page_Data(device, MP_CONTROL, 0x01, use6Byte, &modeData, &modeDataLength))
        {
            ...
            // }
            // now save the fields we care about
            print_str("Control Extension Mode Page\n");
            successfullyReadAtLeastOnePage                          = true;
    ...
    if (SUCCESS ==
        get_SCSI_Mode_Page_Data(device, MP_READ_WRITE_ERROR_RECOVERY, 0, use6Byte, &modeData, &modeDataLength))
    {
        uint16_t blockDescriptorLength = modeData[3];
        uint16_t offset                = MODE_PARAMETER_HEADER_6_LEN + blockDescriptorLength;
        // bool subpageFormat = M_ToBool(modeData[offset + 0] & BIT6);
        ...
        print_str("Read Write Error Recovery Mode Page\n");
        successfullyReadAtLeastOnePage          = true;
        scsiDevInfo->modeData.rwErrRecData.awre = M_ToBool(modeData[offset + 2] & BIT7);
        print_str("\tAutomatic Write Reallocation: ");
        ...
        print_str("\tAutomatic Read Reallocation: ");
    ...
    if (SUCCESS == get_SCSI_Mode_Page_Data(device, MP_RIGID_DISK_GEOMETRY, 0, use6Byte, &modeData, &modeDataLength))
    {
        ...
        print_str("Rigid Disk Geometry Mode Page\n");
        successfullyReadAtLeastOnePage = true;
        scsiDevInfo->modeData.rigidDiskData.numberOfCylinders =
            M_BytesTo4ByteValue(0, modeData[offset + 2], modeData[offset + 3], modeData[offset + 4]);
        ...
        scsiDevInfo->modeData.rigidDiskData.startingCyliderWritePrecompensation =
            M_BytesTo4ByteValue(0, modeData[offset + 6], modeData[offset + 7], modeData[offset + 8]);
        ...
        scsiDevInfo->modeData.rigidDiskData.startingCylinerReducedWriteCurrent =
            M_BytesTo4ByteValue(0, modeData[offset + 9], modeData[offset + 10], modeData[offset + 11]);
        printf("\tStarting Cylinder Reduced Write Current: %" PRIu32 "\n",
               scsiDevInfo->modeData.rigidDiskData.startingCylinerReducedWriteCurrent);
        scsiDevInfo->modeData.rigidDiskData.driveStepRate =
            M_BytesTo2ByteValue(modeData[offset + 12], modeData[offset + 13]);
        printf("\tDrive Step Rate: %" PRIu16 "\n", scsiDevInfo->modeData.rigidDiskData.driveStepRate);
        scsiDevInfo->modeData.rigidDiskData.landingZoneCylinder =
            M_BytesTo4ByteValue(0, modeData[offset + 14], modeData[offset + 15], modeData[offset + 16]);
        printf("\tLanding Zone Cylinder: %" PRIu32 "\n", scsiDevInfo->modeData.rigidDiskData.landingZoneCylinder);
        scsiDevInfo->modeData.rigidDiskData.rpl = M_GETBITRANGE(modeData[offset + 17], 1, 0);
        print_str("\tRotational Position Locking: ");
        switch (scsiDevInfo->modeData.rigidDiskData.rpl)
        {
        case 0:
            print_str("Spindle Synchronization Disabled Or Not Supported\n");
            break;
        case 1:
            print_str("Operates As A Synchronized-Spindle Slave\n");
            break;
        case 2:
            print_str("Operates As A Synchronized-Spindle Master\n");
            break;
        case 3:
            print_str("Operates As A Synchronized-Spindle Master Control\n");
            break;
        default:
            print_str("Unknown\n");
            break;
        ...
        scsiDevInfo->modeData.rigidDiskData.mediumRotationRate =
            M_BytesTo2ByteValue(modeData[offset + 20], modeData[offset + 21]);
        print_str("\tMedium Rotation Rate: ");
    ...
    if (SUCCESS ==
        get_SCSI_Mode_Page_Data(device, MP_INFORMATION_EXCEPTIONS_CONTROL, 0, use6Byte, &modeData, &modeDataLength))
    {
        uint16_t blockDescriptorLength = modeData[3];
        uint16_t offset                = MODE_PARAMETER_HEADER_6_LEN + blockDescriptorLength;
        // bool subpageFormat = M_ToBool(modeData[offset + 0] & BIT6);
        ...
        print_str("Informational Exceptions Control Mode Page\n");
        successfullyReadAtLeastOnePage           = true;
    ...
    if (SUCCESS == get_SCSI_Mode_Page_Data(device, MP_POWER_CONDTION, 0, use6Byte, &modeData, &modeDataLength))
    {
        ...
        print_str("Power Condition Control Mode Page\n");
        successfullyReadAtLeastOnePage                        = true;
        ...
        scsiDevInfo->modeData.powerConditionCtrlData.idleATimer =
            M_BytesTo4ByteValue(modeData[offset + 4], modeData[offset + 5], modeData[offset + 6], modeData[offset + 7]);
        scsiDevInfo->modeData.powerConditionCtrlData.standbyZTimer = M_BytesTo4ByteValue(
            modeData[offset + 8], modeData[offset + 9], modeData[offset + 10], modeData[offset + 11]);
        ...
        if (pageLength >
            0x0A) // need to check this for backwards compatibility since this page existed before all the other timers.
        {
            scsiDevInfo->modeData.powerConditionCtrlData.idleBTimer = M_BytesTo4ByteValue(
                modeData[offset + 12], modeData[offset + 13], modeData[offset + 14], modeData[offset + 15]);
            scsiDevInfo->modeData.powerConditionCtrlData.idleCTimer = M_BytesTo4ByteValue(
                modeData[offset + 16], modeData[offset + 17], modeData[offset + 18], modeData[offset + 19]);
            scsiDevInfo->modeData.powerConditionCtrlData.standbyYTimer = M_BytesTo4ByteValue(
                modeData[offset + 20], modeData[offset + 21], modeData[offset + 22], modeData[offset + 23]);
    ...
    // if () //ATA AND the passthrough hack for not supporting subpages is NOT set
    {
        ...
        // signature vs SATA signature
        {
            ...
            if (SUCCESS == get_SCSI_Mode_Page_Data(device, 0x0A, 0xF1, use6Byte, &modeData, &modeDataLength))
            {
                ...
                print_str("PATA Control Mode Page\n");
                successfullyReadAtLeastOnePage           = true;
        ...
        if (SUCCESS == get_SCSI_Mode_Page_Data(device, 0x1A, 0xF1, use6Byte, &modeData, &modeDataLength))
        {
            ...
            print_str("ATA Power Condition Mode Page\n");
            successfullyReadAtLeastOnePage                 = true;
    ...
    if (!successfullyReadAtLeastOnePage)
    {
        ...
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        set_Console_Colors(true, WARNING_COLOR);
        printf("WARNING: This device does not seem to support any standard mode pages. Multiple pages were attempted, "
...
// TODO: we can clean up the loops in each case to most likely a single loop somewhere, but will need to figure out a
// method to save the data fields we care about...-TJE
static eReturnValues scsi_Log_Information(tDevice* device, ptrScsiDevInformation scsiDevInfo)
{
    set_Console_Colors(true, HEADING_COLOR);
    print_str("\n==========================\n");
    print_str("Testing for SCSI Log Pages\n");
    print_str("==========================\n");
    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    // Make sure on the first log sense, if it fails, we check for invalid operation code. If invalid code, test is over
    // since the command isn't supported. Let the user know about this though!
    DECLARE_ZERO_INIT_ARRAY(uint8_t, supportPages, 255);
    if (SUCCESS ==
        scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, LP_SUPPORTED_LOG_PAGES, 0, 0, supportPages, 255))
    {
        bool hasSubpages = false;
        // we should have the supported logs at this point.
        // Now we need to attempt to read the list of supported subpages as well. WARNING: some device may respond
        // because they don't properly validate reserved fields. Need to catch this!!!
        DECLARE_ZERO_INIT_ARRAY(uint8_t, supportedPagesAndSubpages, 255);
        if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, LP_SUPPORTED_LOG_PAGES_AND_SUBPAGES,
                                          0xFF, 0, supportedPagesAndSubpages, 255))
        {
            ...
            if (memcmp(supportPages, supportedPagesAndSubpages, 255) == 0)
            {
                ...
                printf("This device does NOT report log page subpages properly! Do not attempt to read ANY subpages as "
                       "it only checks the page code!\n");
            }
            else
            {
                // more analysis required
                // The page code should be zero for both. The subpage code should be set to FF for
                // supportedPagesAndSubpages. The lengths MUST be different. Even if there aren't any supported
                // subpages, the lengths cannot be the same since the descriptors are different sizes!
                if (M_GETBITRANGE(supportPages[0], 5, 0) == M_GETBITRANGE(supportedPagesAndSubpages[0], 5, 0))
                {
                    ...
                    {
                        // finally check that the page length for subpages is AT LEAST twice as long as without
                        // subpages. It SHOULD be, but if it isn't we can do one more analysis of the data to see if all
                        // pages are being reported correctly or not.
                        uint16_t supportLen = M_BytesTo2ByteValue(supportPages[2], supportPages[3]);
                        uint16_t supportSLen =
                            M_BytesTo2ByteValue(supportedPagesAndSubpages[2], supportedPagesAndSubpages[3]);
                        ...
                        {
                            set_Console_Colors(true, WARNING_COLOR);
                            printf("WARNING: Length of subpages does not appear to make sense. It should be AT LEAST "
                            ...
                            // TODO: final level of analysis...looping and checking to see if a single page is missing
                            // or not.
                            //       when completed, uncomment the above printf....if this case actually happens, will
                            //       need another hack for this device since it reports correctly formatted data, but
                            //       misses reporting some pages. If this issue is found, a creative way to loop through
                            //       and check both below will be needed. Maybe modify the supportedPagesAndSubpages
                            //       with all the correct information and length before getting down below???
                            hasSubpages = false;
                        }
                    }
                    else
                    {
                        ...
                        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        printf(
                            "This device reports success from asking for subpage list, BUT doesn't report properly\n");
                        hasSubpages                                                 = false;
                        device->drive_info.passThroughHacks.scsiHacks.noLogSubPages = true;
                    }
                }
                ...
                {
                    set_Console_Colors(true, ERROR_COLOR);
                    print_str("ERROR: Data looks completely invalid! Cancelling SCSI log page test!\n");
                    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                    return FAILURE;
                }
            }
        }
        // now loop through the pages &/subpages supported by the device. Save anything interesting to compare against
        // SAT reported data later.
        ...
        for (uint32_t lpSupportOffset = UINT32_C(4);
             lpSupportOffset < C_CAST(uint32_t, (supportedLogPagesLen + 4)) && lpSupportOffset < 255;
             lpSupportOffset += increment)
        {
            ...
            uint16_t logPageLength          = UINT16_C(4); // start here to get full size
            uint8_t  parameterLength        = UINT8_C(0);  // to be used as things are parsed below.
            if (hasSubpages)
            {
            ...
            // TODO: Read the length of the current page/subpage, then read the whole thing. Keep this pointer for the
            // rest of the loop below.
                                              ...
                                              pageToRead, logPageLength))
            {
                ...
                uint8_t* temp =
                    realloc_aligned(pageToRead, logPageLength, logPageLength + 4, device->os_info.minimumAlignment);
                if (temp)
                {
                    pageToRead = temp;
                    if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, pageCode, subPageCode, 0,
                                                      pageToRead, logPageLength + 4))
                    {
                        ...
                        if (pageToRead[0] & BIT6)
                        {
                            ...
                            if (M_GETBITRANGE(pageToRead[0], 5, 0) != pageCode)
                            {
                                set_Console_Colors(true, ERROR_COLOR);
                                printf("ERROR: Expected page %" PRIX8 "h, but got %" PRIX8 "h\n", pageCode,
                                       C_CAST(uint8_t, M_GETBITRANGE(pageToRead[0], 5, 0)));
                                set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                            }
                    ...
                    {
                        set_Console_Colors(true, ERROR_COLOR);
                        printf("ERROR: Failed to read page %02" PRIX8 "h-%02" PRIX8
                               "h. Was reported as supported, but cannot be read.\n",
                               pageCode, subPageCode);
            ...
            // pages for now. Can expand to others if necessary.
            switch (pageCode)
            {
            case 0x00:
                switch (subPageCode)
                {
                case 0x00:
                    print_str("Supported Log Pages\n");
                    break;
                case 0xFF:
                    print_str("Supported Log Pages and Subpages\n");
                    break;
                default:
            ...
            case 0x01:
                ...
                {
                case 0:
                    print_str("Buffer Over-Run/Under-Run\n");
                    break;
                case 0xFF:
            ...
            case 0x02:
                ...
                {
                case 0x00:
                    print_str("Write Error Counters\n");
                    genericLogPagePrintout = false;
                    for (uint32_t offset = UINT32_C(4);
                         offset < C_CAST(uint32_t, logPageLength + 4) && offset < UINT16_MAX;
                         offset += parameterLength + 4)
                    {
                        ...
                        case 0x0000: // Errors corrected without substantial delay
                            print_str("\tErrors corrected without substantial delay: ");
                            break;
                        case 0x0001: // Errors corrected with possible delays
                            print_str("\tErrors corrected with possible delays: ");
                            break;
                        case 0x0002: // total (eg rewrites or rereads)
                            print_str("\tTotal (e.g. rewites or rereads): ");
                            break;
                        case 0x0003: // total errors corrected
                            print_str("\tTotal Errors Corrected: ");
                            break;
                        case 0x0004: // total times correction algorithm processed
                            print_str("\tTotal Times Correction Algorithm Processed: ");
                            break;
                        case 0x0005: // total bytes processed
                            print_str("\tTotal Bytes Processed: ");
                            break;
                        case 0x0006: // total uncorrected errors
                            print_str("\tTotal Uncorrected Errors: ");
                            break;
                        default:
                        ...
                        if (genericCounterOutput)
                        {
                            ...
                            case 4:
                                printf("%" PRIu32 "\n",
                                       M_BytesTo4ByteValue(pageToRead[offset + 5], pageToRead[offset + 6],
                                                           pageToRead[offset + 7], pageToRead[offset + 8]));
                                break;
                            case 8:
                                printf("%" PRIu64 "\n",
                                       M_BytesTo8ByteValue(pageToRead[offset + 5], pageToRead[offset + 6],
                                                           pageToRead[offset + 7], pageToRead[offset + 8],
                                                           pageToRead[offset + 9], pageToRead[offset + 10],
                                                           pageToRead[offset + 11], pageToRead[offset + 12]));
                                break;
                            default:
                                print_Data_Buffer(&pageToRead[offset + 4], parameterLength, true);
                                break;
                         ...
                         offset += parameterLength + 4)
                    {
                        ...
                        case 0x0006: // total uncorrected errors
                            ...
                            case 4:
                                ...
                                break;
                            case 8:
                                scsiDevInfo->logData.readErrorData.totalUncorrectedErrors = M_BytesTo8ByteValue(
                                    pageToRead[offset + 5], pageToRead[offset + 6], pageToRead[offset + 7],
                                    pageToRead[offset + 8], pageToRead[offset + 9], pageToRead[offset + 10],
                                    pageToRead[offset + 11], pageToRead[offset + 12]);
                                break;
                            }
                            printf("%" PRIu64 "\n", scsiDevInfo->logData.readErrorData.totalUncorrectedErrors);
            ...
            case 0x05:
                ...
                {
                case 0x00:
                    print_str("Verify Error Counters\n");
                    genericLogPagePrintout = false;
                    for (uint32_t offset = UINT32_C(4);
                         offset < C_CAST(uint32_t, logPageLength + 4) && offset < UINT16_MAX;
                         offset += parameterLength + 4)
            ...
            case 0x0B:
                ...
                {
                case 0x00:
                    print_str("Last n Deferred Error or Asynchronous Events\n");
                    break;
                case 0x01:
                    print_str("Last n Inquiry Data Changed\n");
                    break;
                case 0x02:
                    print_str("Last n Mode Page Data Changed\n");
                    break;
                case 0xFF:
                    printf("Supported subpages of page code %02" PRIX8 "h\n", pageCode);
                    break;
                ...
                case 0x00:
                         ...
                         offset += parameterLength + 4)
                    {
                        ...
                        case 0x0000: // Temperature
                            print_str("\tTemperature (C): ");
                            if (pageToRead[offset + 5] == UINT8_MAX)
                            {
                                print_str("Invalid reading.\n");
                            }
                            else
                            {
                                printf("%" PRIu8 "\n", pageToRead[offset + 5]);
                        ...
                        case 0x0001: // Reference Temperature
                            print_str("\tReference Temperature (C): ");
                            if (pageToRead[offset + 5] == UINT8_MAX)
                            {
                                print_str("Invalid reading.\n");
                            }
                    ...
                    break;
                case 0x01:
                    print_str("Environmental Reporting\n");
                    // TODO: Parse out this page
                    break;
                case 0x02:
                    print_str("Environmental Limits\n");
                    break;
                case 0xFF:
                    printf("Supported subpages of page code %02" PRIX8 "h\n", pageCode);
                    break;
                         ...
                         offset += parameterLength + 4)
                    {
                        ...
                        case 0x0001: //
                            printf("\tDate Of Manufacture: Week %c%c, %c%c%c%c\n", pageToRead[offset + 8],
                                   pageToRead[offset + 9], pageToRead[offset + 4], pageToRead[offset + 5],
                                   pageToRead[offset + 6], pageToRead[offset + 7]);
                            break;
                        case 0x0002: //
                        ...
                        case 0x0003:
                            printf("\tSpecified Cycle Count Over Device Lifetime: %" PRIu32 "\n",
                                   M_BytesTo4ByteValue(pageToRead[offset + 4], pageToRead[offset + 5],
                                                       pageToRead[offset + 6], pageToRead[offset + 7]));
                            break;
                        case 0x0004:
                            printf("\tAccumulated Start-Stop Cycles: %" PRIu32 "\n",
                                   M_BytesTo4ByteValue(pageToRead[offset + 4], pageToRead[offset + 5],
                                                       pageToRead[offset + 6], pageToRead[offset + 7]));
                            scsiDevInfo->logData.startStopData.accumulatedStartStopCycles =
                                M_BytesTo4ByteValue(pageToRead[offset + 4], pageToRead[offset + 5],
                                                    pageToRead[offset + 6], pageToRead[offset + 7]);
                        ...
                        case 0x0006:
                            printf("\tAccumulated Load-Unload Cycles: %" PRIu32 "\n",
                                   M_BytesTo4ByteValue(pageToRead[offset + 4], pageToRead[offset + 5],
                                                       pageToRead[offset + 6], pageToRead[offset + 7]));
                            scsiDevInfo->logData.startStopData.accumulatedLoadUnloadCycles =
                                M_BytesTo4ByteValue(pageToRead[offset + 4], pageToRead[offset + 5],
                                                    pageToRead[offset + 6], pageToRead[offset + 7]);
                            scsiDevInfo->logData.gotStartStop = true;
                            break;
            ...
            case 0x14:
                ...
                {
                case 0x01:
                    print_str("Zoned Block Device Statistics\n");
                    // TODO: Parse out this page
                    break;
                case 0xFF:
                    printf("Supported subpages of page code %02" PRIX8 "h\n", pageCode);
            ...
            case 0x15:
                ...
                {
                case 0x00:
                    print_str("Background Scan Results\n");
                    genericLogPagePrintout = false;
                    for (uint32_t offset = UINT32_C(4);
                         offset < C_CAST(uint32_t, logPageLength + 4) && offset < UINT16_MAX;
                         offset += parameterLength + 4)
                    {
                        ...
                        {
                        case 0x0000:
                            print_str("\tBackground Scan Status\n");
                            ...
                            {
                            case 0x00:
                                print_str("No background scan operation is active\n");
                                break;
                            case 0x01:
                                print_str("A background medium scan operation is active\n");
                                break;
                            case 0x02:
                                print_str("A background pre-scan operation is active\n");
                                break;
                            case 0x03:
                                print_str("A background scan operation was halted due to a fatal error\n");
                                break;
                            case 0x04:
                                printf("A background scan operation was halted due to a vendor specific pattern of "
                                       "errors\n");
                                break;
                            case 0x05:
                                printf("A background scan operation was halted due to the medium being formatted "
                            ...
                            case 0x06:
                                ...
                                break;
                            case 0x07:
                                printf("A background scan operation was halted due to the temperature being out of the "
                                       "allowed range\n");
                                break;
                            case 0x08:
                                printf("Background medium scan operations are enabled and no background medium scan "
                                       "operation is active\n");
                                break;
                            case 0x09:
                                printf("A background scan operation was halted due to the S_L_Full bit being set to "
                                       "one in the Background Control mode page and the background scan results list "
                                       "being full\n");
                                break;
                            case 0x0A:
                                printf("A background pre-scan operation was halted due ot the Background Pre-scan Time "
                        ...
                        default:
                            if (parameterCode > 0 && parameterCode <= 0x0800)
                            {
                                ...
                                print_str("\t\tReassign Status: ");
                                switch (M_Nibble1(pageToRead[offset + 8]))
                                {
                                case 0x1:
                                    print_str("The LBA has not yet been reassigned\n");
                                    break;
                                case 0x2:
                                    print_str("The device server performed automatic read reassignment for the LBA\n");
                                    break;
                                case 0x4:
                                    printf("The device server's attempt to perform automatic read reassignment failed. "
                                           "The LBA may or may not now have an uncorrectable error\n");
                                    break;
                                case 0x5:
                                    printf("The error was corrected by the device server rewriting the LBA without "
                                           "performing a reassign operation\n");
                                    break;
                                case 0x6:
                                    printf("An application client caused automatic write reassignment for the LBA with "
                                           "a command performing a write operation or an application client caused an "
                                           "unmap operation for the LBA\n");
                                    break;
                                case 0x7:
                                    printf("An application client caused a reassign operation for the LBA with a "
                                           "reassign block command or an application client caused an unmap operation "
                                           "for the LBA\n");
                                    break;
                                case 0x8:
                                    printf("An application client's request for a reassign operation for the LBA with "
                                           "a reassign blocks command failed. The logical block referenced by the LBA "
                                           "may or may not still have an uncorrectable error\n");
                                    break;
                                default:
                                ...
                                printf("\t\tLogical Block Address: %" PRIu64 "\n",
                                       M_BytesTo8ByteValue(pageToRead[offset + 16], pageToRead[offset + 17],
                                                           pageToRead[offset + 18], pageToRead[offset + 19],
                                                           pageToRead[offset + 20], pageToRead[offset + 21],
                                                           pageToRead[offset + 22], pageToRead[offset + 23]));
                            }
                ...
                case 0x01:
                         ...
                         offset += parameterLength + 4)
                    {
                        ...
                        default:
                            if (parameterCode > 0 && parameterCode <= 0xF000)
                            {
                                ...
                                uint32_t poh = M_BytesTo4ByteValue(pageToRead[offset + 4], pageToRead[offset + 5],
                                                                   pageToRead[offset + 6], pageToRead[offset + 7]);
                                print_str("\t\tAccumulated Power On Hours: ");
                                if (poh == UINT32_MAX)
                                {
                                    print_str("Unknown\n");
                                }
                                ...
                                printf("\t\tLogical Block Address: %" PRIu64 "\n",
                                       M_BytesTo8ByteValue(pageToRead[offset + 8], pageToRead[offset + 9],
                                                           pageToRead[offset + 10], pageToRead[offset + 11],
                                                           pageToRead[offset + 12], pageToRead[offset + 13],
                                                           pageToRead[offset + 14], pageToRead[offset + 15]));
                            }
                            else if (parameterCode < 0x8000)
                            {
                    ...
                    break;
                case 0x02:
                    print_str("Background Operation\n");
                    break;
                case 0x03:
                    print_str("LPS Misalignment\n");
                    break;
                case 0xFF:
                    printf("Supported subpages of page code %02" PRIX8 "h\n", pageCode);
                    break;
            ...
            case 0x16:
                ...
                {
                case 0x00:
                    print_str("ATA Pass-through Results\n");
                    genericLogPagePrintout = false;
                    for (uint32_t offset = UINT32_C(4);
                         offset < C_CAST(uint32_t, logPageLength + 4) && offset < UINT16_MAX;
                         offset += parameterLength + 4)
            ...
            case 0x18:
                ...
                {
                case 0x00:
                    print_str("Protocol Specific Port\n");
                    break;
                case 0xFF:
                    printf("Supported subpages of page code %02" PRIX8 "h\n", pageCode);
                    break;
            ...
            case 0x19:
                ...
                {
                case 0x00:
                    print_str("General Statistics and Performance\n");
                    genericLogPagePrintout = false;
                    for (uint32_t offset = UINT32_C(4);
                         offset < C_CAST(uint32_t, logPageLength + 4) && offset < UINT16_MAX;
                         offset += parameterLength + 4)
                    {
                        ...
                        {
                        case 0x0001:
                            print_str("\tGeneral Access Statitics and Performance\n");
                            scsiDevInfo->logData.gotGeneralStatistics                       = true;
                            scsiDevInfo->logData.generalStatisticsData.numberOfReadCommands = M_BytesTo8ByteValue(
                                pageToRead[offset + 4], pageToRead[offset + 5], pageToRead[offset + 6],
                                pageToRead[offset + 7], pageToRead[offset + 8], pageToRead[offset + 9],
                                pageToRead[offset + 10], pageToRead[offset + 11]);
                            scsiDevInfo->logData.generalStatisticsData.numberOfWriteCommands = M_BytesTo8ByteValue(
                                pageToRead[offset + 12], pageToRead[offset + 13], pageToRead[offset + 14],
                                pageToRead[offset + 15], pageToRead[offset + 16], pageToRead[offset + 17],
                                pageToRead[offset + 18], pageToRead[offset + 19]);
                            scsiDevInfo->logData.generalStatisticsData.numberOfLogicalBlocksReceived =
                                M_BytesTo8ByteValue(pageToRead[offset + 20], pageToRead[offset + 21],
                                                    pageToRead[offset + 22], pageToRead[offset + 23],
                                                    pageToRead[offset + 24], pageToRead[offset + 25],
                                                    pageToRead[offset + 26], pageToRead[offset + 27]);
                            scsiDevInfo->logData.generalStatisticsData.numberOfLogicalBlocksTransmitted =
                                M_BytesTo8ByteValue(pageToRead[offset + 28], pageToRead[offset + 29],
                                                    pageToRead[offset + 30], pageToRead[offset + 31],
                                                    pageToRead[offset + 32], pageToRead[offset + 33],
                                                    pageToRead[offset + 34], pageToRead[offset + 35]);
                            ...
                            printf("\t\tRead Command Processing Intervals: %" PRIu64 "\n",
                                   M_BytesTo8ByteValue(pageToRead[offset + 36], pageToRead[offset + 37],
                                                       pageToRead[offset + 38], pageToRead[offset + 39],
                                                       pageToRead[offset + 40], pageToRead[offset + 41],
                                                       pageToRead[offset + 42], pageToRead[offset + 43]));
                            printf("\t\tWrite Command Processing Intervals: %" PRIu64 "\n",
                                   M_BytesTo8ByteValue(pageToRead[offset + 44], pageToRead[offset + 45],
                                                       pageToRead[offset + 46], pageToRead[offset + 47],
                                                       pageToRead[offset + 48], pageToRead[offset + 49],
                                                       pageToRead[offset + 50], pageToRead[offset + 51]));
                            printf("\t\tWeighted Number Of Read Commands Plus Write Commands: %" PRIu64 "\n",
                                   M_BytesTo8ByteValue(pageToRead[offset + 52], pageToRead[offset + 53],
                                                       pageToRead[offset + 54], pageToRead[offset + 55],
                                                       pageToRead[offset + 56], pageToRead[offset + 57],
                                                       pageToRead[offset + 58], pageToRead[offset + 59]));
                            printf("\t\tWeighted Read Command Processing Plus Write Command Processing: %" PRIu64 "\n",
                                   M_BytesTo8ByteValue(pageToRead[offset + 60], pageToRead[offset + 61],
                                                       pageToRead[offset + 62], pageToRead[offset + 63],
                                                       pageToRead[offset + 64], pageToRead[offset + 65],
                                                       pageToRead[offset + 66], pageToRead[offset + 67]));
                            break;
                        case 0x0002:
                            print_str("\tIdle Time\n");
                            printf("\t\tIdle Time Intervals: %" PRIu64 "\n",
                                   M_BytesTo8ByteValue(pageToRead[offset + 4], pageToRead[offset + 5],
                                                       pageToRead[offset + 6], pageToRead[offset + 7],
                                                       pageToRead[offset + 8], pageToRead[offset + 9],
                                                       pageToRead[offset + 10], pageToRead[offset + 11]));
                            break;
                        case 0x0003:
                            print_str("\tTime Interval\n");
                            printf("\t\tTimer Interval Descriptor: %" PRIu32 " x 10^-%" PRIu32 "\n",
                                   M_BytesTo4ByteValue(pageToRead[offset + 8], pageToRead[offset + 9],
                                                       pageToRead[offset + 10], pageToRead[offset + 11]),
                                   M_BytesTo4ByteValue(pageToRead[offset + 4], pageToRead[offset + 5],
                                                       pageToRead[offset + 6], pageToRead[offset + 7]));
                            break;
                        case 0x0004:
                            ...
                            printf("\t\tNumber Of Write FUA Commands: %" PRIu64 "\n",
                                   M_BytesTo8ByteValue(pageToRead[offset + 12], pageToRead[offset + 13],
                                                       pageToRead[offset + 14], pageToRead[offset + 15],
                                                       pageToRead[offset + 16], pageToRead[offset + 17],
                                                       pageToRead[offset + 18], pageToRead[offset + 19]));
                        ...
                        default:
                            ...
                            {
                                printf("Vendor Specific General Statistics and Performance parameter code: %04" PRIX16
                                       "h\n",
                                       parameterCode);
                                print_Data_Buffer(&pageToRead[offset + 4], parameterLength, true);
                            }
                    ...
                    break;
                case 0x20:
                    print_str("Cache Memory Statistics\n");
                    break;
                case 0xFF:
                    printf("Supported subpages of page code %02" PRIX8 "h\n", pageCode);
                    break;
                default:
                    ...
                    {
                        printf("Unknown Group Statistics and Performance Page and Subpage code: %02" PRIX8 "h-%02" PRIX8
                               "h\n",
                               pageCode, subPageCode);
                    }
                    else
            ...
            case 0x1A:
                ...
                {
                case 0x00:
                    print_str("Power Condition Transitions\n");
                    break;
                case 0xFF:
                    printf("Supported subpages of page code %02" PRIX8 "h\n", pageCode);
                    break;
            ...
            case 0x2F:
                ...
                case 0x00:
                         ...
                         offset += parameterLength + 4)
                    {
                        ...
                        {
                        case 0x0000:
                            print_str("\tInformational Exceptions General\n");
                            ...
                            if (scsiDevInfo->logData.informationalExceptionsData.mostRecentTemperatureReading ==
                                UINT8_MAX)
                            {
                                printf("\t\tMost Recent Temperature Reading: Invalid reading due to sensor failure or "
                                       "other condition\n");
                            }
                            else
                            {
            ...
            default:
                ...
                {
                    // reserved
                    print_str("Reserved Page code 3Fh\n");
                }
                else
                {
    ...
    else
    {
        if (is_Invalid_Opcode(device->drive_info.lastCommandSenseData, SPC3_SENSE_LEN))
        {
            ...
            set_Console_Colors(true, NOTE_COLOR);
            print_str("NOTE: Skipping SCSI Log test since device reported invalid operation code.\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        }
        else
        {
        ...
        set_Console_Colors(true, HACK_COLOR);
        print_str("HACK FOUND: NLP\n");
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    }
    return SUCCESS;
}

typedef struct s_scsiRWSupport
{
    bool sixBytes;
    bool tenBytes;
    bool twelveBytes;
...
static eReturnValues scsi_Read_Check(tDevice*         device,
                                     bool             zeroLengthTransfers,
                                     ptrScsiRWSupport rwSupport,
                                     bool             testZeroLengthTransfersToo)
{
    if (!device || !rwSupport)
    {
        return BAD_PARAMETER;
    ...
    {
        print_str("\n======================================================\n");
        print_str("Checking SCSI Read/Write Command Support (Zero Length)\n");
        print_str("======================================================\n");
    }
    else
    {
        print_str("\n========================================\n");
        print_str("Checking SCSI Read/Write Command Support\n");
        print_str("========================================\n");
    }
    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    ...
    {
        transferLength = 1;
        ptrData = M_REINTERPRET_CAST(uint8_t*, safe_calloc_aligned(device->drive_info.deviceBlockSize, sizeof(uint8_t),
                                                                   device->os_info.minimumAlignment));
    ...
    {
        set_Console_Colors(true, NOTE_COLOR);
        print_str("NOTE: Skipping R6 since zero length has a different meaning than other commands\n");
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    }

    // read 10
    if (SUCCESS == scsi_Read_10(device, 0, false, false, false, 0, 0, C_CAST(uint16_t, transferLength), ptrData,
                                transferLengthBytes))
    {
        rwSupport->tenBytes = true;
        set_Console_Colors(true, HACK_COLOR);
        print_str("HACK FOUND: RW10\n");
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        device->drive_info.passThroughHacks.scsiHacks.readWrite.available = true;
    ...
    {
        set_Console_Colors(true, ERROR_COLOR);
        print_str("ERROR: Failed to find any support read commands by the device!\n");
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        rwSupport->nonZeroSectorCountRequired = true;
        set_Console_Colors(true, HACK_COLOR);
        print_str("HACK FOUND: NZTL\n"); // non-zero transfer length
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
...
typedef struct s_otherSCSICmdSupport
{
...
static eReturnValues other_SCSI_Cmd_Support(tDevice* device, ptrOtherSCSICmdSupport scsiCmds)
{
    ...
    set_Console_Colors(true, HEADING_COLOR);
    print_str("\n===========================\n");
    print_str("Testing Other SCSI Commands\n");
    print_str("===========================\n");
    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    ...
    {
        set_Console_Colors(true, ERROR_COLOR);
        print_str("ERROR: Test unit ready command total failure. This is a critical command!\n");
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    }
    ...
    {
        set_Console_Colors(true, ERROR_COLOR);
        print_str("ERROR: Report LUNS failed. This is a critical command!\n");
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    }
    ...
    if (SUCCESS == scsi_SecurityProtocol_In(device, 0, 0, false, 512, scsiDataBytes))
    {
        ...
        set_Console_Colors(true, HACK_COLOR);
        print_str("HACK FOUND: SECPROT\n");
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    }
    else
    {
        // Check if we had a invalid operation code, or if we need the inc512 bit set as some things may be requiring
        // that bit to be set!
        if (does_Sense_Data_Show_Invalid_Field_In_CDB(device))
        {
            ...
            if (SUCCESS == scsi_SecurityProtocol_In(device, 0, 0, true, 1, scsiDataBytes))
            {
                ...
                device->drive_info.passThroughHacks.scsiHacks.securityProtocolSupported  = true;
                set_Console_Colors(true, HACK_COLOR);
                print_str("HACK FOUND: SECPROTI512\n");
                set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
            }
            else
            {
                set_Console_Colors(true, WARNING_COLOR);
                printf("WARNING: Device reported in a way that suggests that security protocol commands work, but no "
        ...
        {
            set_Console_Colors(true, WARNING_COLOR);
            printf("WARNING: Security protocol in failed. Access to device security subsystems may be inaccessible or "
                   "limited!\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        }
    }
    ...
    else
    {
        ...
        {
            set_Console_Colors(true, WARNING_COLOR);
            printf("WARNING: Reporting supported operation codes failed! This command does not appear to be known by "
                   "the device.\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        }
        ...
        {
            set_Console_Colors(true, WARNING_COLOR);
            printf("WARNING: Reporting all supported operation codes is not supported! Will attempt requesting a "
    ...
    if (!does_Sense_Data_Show_Invalid_OP(device))
    {
        ...
        {
            set_Console_Colors(true, WARNING_COLOR);
            printf("WARNING: Reporting support for single requested operation codes failed! Unable to request command "
    ...
    // send diagnostic for default device self test. Wait up to 5 minutes for this command since some devices could take
    // longer to process this
    // TODO: it is possible for self-test to fail, which we should catch since it performed the test and didn't return
    // invalid operation code or invalid field in CDB
    /*eReturnValues selfTestResult =*/scsi_Send_Diagnostic(device, 0, 0, 1, 0, 0, 0, M_NULLPTR, 0, 5 * 60);
    if (!does_Sense_Data_Show_Invalid_OP(device))
    {
        scsiCmds->sendDiagnostic = true;
        print_str("Device is capable of performing default self-test\n");
    }
    else
    {
        set_Console_Colors(true, WARNING_COLOR);
        print_str("WARNING: Default self-test is not available.\n");
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    }
...
static eReturnValues scsi_Error_Handling_Test(tDevice* device, double* badCommandRelativeTimeToGood)
{
    ...
    set_Console_Colors(true, HEADING_COLOR);
    print_str("\n==============================================\n");
    print_str("Testing Error Handling Of Unsupported Commands\n");
    print_str("==============================================\n");
    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
...
// Now that a base is established, start sending 10 or more unsupported commands and check how long the last one took.
// If it's near the baseline, it's fine If not, and the time is significantly longer, then we know that sending test
// unit ready commands after each failed command is necessary for it to perform well.
#define MAX_COMMANDS_TO_TRY 16
    DECLARE_ZERO_INIT_ARRAY(uint64_t, commandTimes, MAX_COMMANDS_TO_TRY + 1);
    uint8_t commandIter = UINT8_C(0);
    ...
    ++commandIter;
    // Now try one that is likely to not be supported and return invalid field in CDB (vendor specific page of some kind
    // or another...try something in middle of range to hopefully avoid something that may actually be implented.)
    eReturnValues ret      = SUCCESS;
    uint8_t       pageCode = UINT8_C(0x2B);
    uint8_t       subpage  = UINT8_C(0x01);
    ...
    for (uint8_t counter = UINT8_C(0);
         counter < MAX_COMMANDS_TO_TRY && commandIter < MAX_COMMANDS_TO_TRY && ret != OS_COMMAND_TIMEOUT; ++counter)
    {
        ret = scsi_Mode_Sense_10(device, pageCode, 255, subpage, true, false, MPC_CURRENT_VALUES, dataBuffer);
        commandTimes[commandIter] = device->drive_info.lastCommandTimeNanoSeconds;
    ...
    uint64_t averageFromBadCommands = UINT64_C(0);
    uint8_t  totalCmds              = UINT8_C(0);
    for (uint8_t badIter = UINT8_C(4); badIter < MAX_COMMANDS_TO_TRY; ++badIter, ++totalCmds)
    {
    ...
    if (ret == OS_COMMAND_TIMEOUT ||
        xTimesHigher > 3.0) // 2 is pretty bad, but not uncommon, so moved up to 3.0 to be a little conservative. Don't
                            // want or need to enable this on everything. - TJE
    {
        // check just how much higher it is...if more than 2x higher, then there is an issue
        // Checking the last one because it is the most important. For this test, it should get much MUCH longer quickly
        // and stay that way.
    ...
    {
        set_Console_Colors(true, LIKELY_HACK_COLOR);
        printf("Likely HACK FOUND: TURF%" PRIu8 "\n", 33); // setting 33...this should be sooo much higher and worse
                                                           // that this should remain true for a long long time.
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        device->drive_info.passThroughHacks.testUnitReadyAfterAnyCommandFailure = true;
...
static eReturnValues sct_GPL_Test(tDevice* device, bool smartSupported, bool gplSupported, bool sctSupported)
{
    ...
    if (sctSupported && smartSupported && gplSupported)
    {
        ...
        printf("If this test hangs the device, it will need to be unplugged and the tool rerun without the sctgpl "
               "test.\n");
        printf("If the device hangs, the additional hack \"SCTSM\" must be used with this device for full "
               "functionality.\n");

        // SMART first
        print_str("\tTesting with SMART read log\n");
        if (SUCCESS == ata_SMART_Read_Log(device, ATA_SCT_COMMAND_STATUS, sctStatus, 512))
        {
            print_str("\t    Successfully read SCT status with SMART Read Log!\n");
            smartWorked = true;
        }
        else
        {
            set_Console_Colors(true, ERROR_COLOR);
            print_str("ERROR: Something went wrong trying to read the SCT status log!!!\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        }
        // now GPL
        print_str("\tTesting with read log ext\n");
        if (SUCCESS == ata_Read_Log_Ext(device, ATA_SCT_COMMAND_STATUS, 0, sctStatus, 512,
                                        (device->drive_info.ata_Options.dmaMode != ATA_DMA_MODE_NO_DMA &&
                                        ...
                                        0))
        {
            print_str("\t    Successfully read SCT status with Read Log Ext!\n");
        }
        else
        {
            if (smartWorked)
            {
                set_Console_Colors(true, HACK_COLOR);
                print_str("HACK FOUND: SCTSM\n");
                set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                print_str("\t\tA retest is recommended with this test turned off. Devices with this hack\n");
                print_str("\t\toften do not recover properly and need a full power cycle once this issue is\n");
                print_str("\t\ttested and identified.\n");
    ...
    {
        print_str("Skipping SCTGPL test because device does not support features required for this test.\n");
        print_str("\t");
        if (!sctSupported)
        {
            print_str("(SCT Not Supported) ");
        }
        if (!smartSupported)
        {
            print_str("(SMART Not Supported) ");
        }
        if (!gplSupported)
        {
            print_str("(GPL Not Supported) ");
        }
        print_str("\n");
    }
    return NOT_SUPPORTED;
...
static void setup_ATA_ID_Info(ptrPassthroughTestParams inputs,
                              bool*                    smartSupported,
                              ...
                              bool*                    sctSupported)
{
    ...
    fill_ATA_Strings_From_Identify_Data(identifyData, inputs->device->drive_info.bridge_info.childDriveMN,
                                        inputs->device->drive_info.bridge_info.childDriveSN,
    ...
    {
        inputs->device->drive_info.bridge_info.childDeviceMaxLba =
            M_BytesTo4ByteValue(identifyData[123], identifyData[122], identifyData[121], identifyData[120]);
    }
...
static eReturnValues sat_Ext_Cmd_With_A1_When_Possible_Test(tDevice* device)
{
    ...
    if (device->drive_info.ata_Options.generalPurposeLoggingSupported)
    {
        ...
        print_str("Testing if it is possible to issue limited Ext (48bit) commands with A1 CDB\n");
        ...
        if (SUCCESS == ata_Read_Log_Ext(device, ATA_LOG_DIRECTORY, 0, log, 512, false, 0))
        {
            ...
            set_Console_Colors(true, HACK_COLOR);
            print_str("HACK FOUND: A1EXT\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
            ret = SUCCESS;
        }
        else
        {
            print_str("\tUnable to use limited 48bit commands with A1h\n");
...
static bool test_SAT_Capabilities(ptrPassthroughTestParams inputs, ptrScsiDevInformation scsiInformation)
{
    set_Console_Colors(true, HEADING_COLOR);
    print_str("\n=====================================\n");
    print_str("Checking SAT ATA-passthrough commands\n");
    print_str("=====================================\n");
    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    ...
    if (SUCCESS == satRet || WARN_INVALID_CHECKSUM == satRet)
    {
        ...
        print_str("SAT 12-byte passthrough supported\n");
        satSupported        = true;
        twelveByteSupported = true;
    }
    inputs->device->drive_info.passThroughHacks.ataPTHacks.a1NeverSupported = true; // forcing 16B command
    satRet =
        sat_Test_Identify(inputs->device, identifyData, 512,
                          16); // NOTE: On some devices this will fail because for some stupid reason, these devices
                               // will only issue a 48bit command with 16B commands and reject others...no idea why-TJE
    if (SUCCESS == satRet || WARN_INVALID_CHECKSUM == satRet)
    {
        // TODO: Validate Identify data!
        print_str("SAT 16-byte passthrough supported\n");
        satSupported         = true;
    ...
    if (satSupported)
    {
        ...
        if (satRet == SUCCESS)
        {
            print_str("SAT 32-byte passthrough supported\n");
            satSupported = true;
        }
        else
        {
        ...
        // TODO: one more test to see if 48bit commands NOT setting ext register is possible/ Try something like
        // readlogext to get the supported logs since that won't set ext registers.
        ...
        if (!sixteenByteSupported && !doNotRetry16BSAT)
        {
                ...
                inputs->device->drive_info.ata_Options.generalPurposeLoggingSupported)
            {
                ...
                {
                    print_str("SAT 16-byte passthrough supported, but only for 48bit commands!\n");
                    satSupported         = true;
                    sixteenByteSupported = true;
                }
                else
        ...
        else if (!sixteenByteSupported &&
                 inputs->device->drive_info.ata_Options.fourtyEightBitAddressFeatureSetSupported)
        {
            set_Console_Colors(true, WARNING_COLOR);
            print_str("WARNING: This device is not able to pass-through 48 bit (extended) commands!\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        }
        ...
        {
            // if TPSIU worked for identify, we need to try another command, a read, to ensure that it actually works
            // for other commands. This was added after testing yet another USB bridge that did something odd and
            // different that doesn't really work well
            uint8_t* data = M_REINTERPRET_CAST(
                uint8_t*, safe_calloc_aligned(inputs->device->drive_info.deviceBlockSize * 1, sizeof(uint8_t),
                                              inputs->device->os_info.minimumAlignment));
            if (data)
            {
                ...
                inputs->device->drive_info.passThroughHacks.ataPTHacks.alwaysUseTPSIUForSATPassthrough = true;
                // Before declaring TPSIU support to be always used, additionally test a read passthrough command to see
                // if that works. - TJE Using a PIO read command since the DMA test hasn't been performed yet. This is a
                // single sector and should be OK to do at this point. - TJE
                eReturnValues tpsiuRet =
                    ata_Read_Sectors(inputs->device, 0, data, 1, inputs->device->drive_info.deviceBlockSize * 1, use48);
                if (SUCCESS == tpsiuRet && !does_Sense_Data_Show_Invalid_Field_In_CDB(inputs->device))
                {
                    set_Console_Colors(true, HACK_COLOR);
                    print_str("HACK FOUND: TPSIU\n"); // alwaysUseTPSIUForSATPassthrough
                    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                }
                else
                {
                    set_Console_Colors(true, HACK_COLOR);
                    print_str("HACK FOUND: TPID\n");
                    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
            ...
            {
                set_Console_Colors(true, WARNING_COLOR);
                print_str("WARNING: Unable to allocate memory and fully test TPSIU capability.\n");
                set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                inputs->device->drive_info.passThroughHacks.ataPTHacks.alwaysUseTPSIUForSATPassthrough = false;
            }
        }
        ...
        {
            print_str("Skipping DMA check as device does not report any support for DMA mode commands\n");
            print_str("It is strongly recommended to retest with a device that does support DMA mode commands\n");
        }
        ...
        if (!inputs->noMultiSectorPIOTest)
        {
            ...
            {
                // print_str("Multi-sector PIO commands seem to work properly\n");
        ...
        //  Will try SMART, get native max LBA first. Then try sending the drive to standby and checking the power mode,
        //  then spinning it up and checking again. Need to make sure we look for ext RTFRs as well.
        ...
        {
            return_Response_Info_Test(inputs->device, smartSupported, smartLoggingSupported,
                                      (inputs->testPotentiallyDeviceHangingCommands &&
                                       inputs->hangCommandsToTest.returnResponseInforWithoutTdir)
                                          ? true
                                          : false);
        }
        ...
        {
            sct_GPL_Test(inputs->device, smartSupported,
                         inputs->device->drive_info.ata_Options.generalPurposeLoggingSupported, sctSupported);
        }

        set_Console_Colors(true, HEADING_COLOR);
        print_str("\n====================================\n");
        print_str("Comparing ATA and SCSI reported data\n");
        print_str("====================================\n");
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        // Here we will compare the reported information by the bridge from SCSI commands to what the ATA Identify data
...
#define PASSTHROUGH_TEST_SCSI_PROD_ID_LEN 17
        ...
        {
            print_str("\tSAT Compliant product ID reported\n");
        }
        else
        {
            set_Console_Colors(true, WARNING_COLOR);
            printf("\tWARNING: Non-SAT Compliant product ID reported. This may be a \"branded\" product or "
                   "non-compliant translator.\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
            print_str("\t  Checking common non-compliant reporting.\n");
            if (strstr(inputs->device->drive_info.bridge_info.childDriveMN, scsiInformation->inquiryData.productId) &&
                strstr(inputs->device->drive_info.bridge_info.childDriveMN, scsiInformation->inquiryData.vendorId))
            {
...
#define PASSTHROUGH_TEST_FULL_MN_LENGTH 42
                ...
                {
                    print_str("\t\tTranslator put full ATA MN in combination of Vendor and Product ID fields.\n");
                }
            }
            else
            {
                print_str("\t\tUnknown reporting format.\n");
            }
        }
        if (safe_strlen(scsiInformation->vpdData.unitSN))
        {
            print_str("\tChecking unit serial number\n");
            if (strstr(scsiInformation->vpdData.unitSN,
                       inputs->device->drive_info.bridge_info
                           .childDriveSN)) // using strstr since we remove whitespace from childdriveSN, but not the
                                           // unit serial number we saved.
            {
                print_str("\tUnit serial number reported in SAT compliant translation!\n");
            }
            else
            {
                print_str("\tUnit serial number not reported in a SAT compliant way.\n");
            }
        }
        else
        {
            print_str("\tNo Unit serial number reported. Skipping this check\n");
        }
        ...
        if (scsiInformation->vpdData.gotBlockCharacteristicsVPDPage)
        {
                    ...
                    scsiInformation->vpdData.blockCharacteristicsData.rotationRate)
            {
                ...
                {
                    set_Console_Colors(true, ERROR_COLOR);
                    print_str("ERROR: Even after byteswapping, the rotation rate still doesn't match!!!\n");
                    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                }
                else
                {
                    set_Console_Colors(true, WARNING_COLOR);
                    printf("It seems like the RPM was reported without byteswapping it first! Fix this in the "
                           "translator firmware!\n");
                    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                }
            }

            // Check form factor (if SCSI reported it)..if this doesn't match, show a warning since this page was
            // otherwise reported.
            if (M_Nibble0(inputs->device->drive_info.IdentifyData.ata.Word168) != 0 &&
                M_Nibble0(inputs->device->drive_info.IdentifyData.ata.Word168) !=
                    scsiInformation->vpdData.blockCharacteristicsData.formFactor)
            {
                set_Console_Colors(true, ERROR_COLOR);
            ...
            le16_to_host(inputs->device->drive_info.IdentifyData.ata.Word087) & BIT8)
        {
            if (scsiInformation->vpdData.gotDeviceIDVPDPage)
            {
                ...
                for (uint8_t iter = UINT8_C(0); iter < scsiInformation->vpdData.designatorCount; ++iter)
                {
                    if (scsiInformation->vpdData.designators[iter].valid &&
                        scsiInformation->vpdData.designators[iter].designatorType == 3) // need NAA type for WWN
                    {
                        if (scsiInformation->vpdData.designators[iter].designatorLength == 8)
                        {
                            uint64_t scsiWWN =
                                M_BytesTo8ByteValue(scsiInformation->vpdData.designators[iter].designator[0],
                                                    scsiInformation->vpdData.designators[iter].designator[1],
                                                    scsiInformation->vpdData.designators[iter].designator[2],
                                                    scsiInformation->vpdData.designators[iter].designator[3],
                                                    scsiInformation->vpdData.designators[iter].designator[4],
                                                    scsiInformation->vpdData.designators[iter].designator[5],
                ...
                {
                    set_Console_Colors(true, ERROR_COLOR);
                    print_str("ERROR: WWN was not found in device identification VPD page!\n");
                    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                }
            }
            else
            {
                set_Console_Colors(true, WARNING_COLOR);
                print_str("WARNING: Device didn't report device ID VPD page, cannot check for WWN translation!\n");
                set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
            }
        }

        // check block sizes and maxLBA values
        // Throw a warning that the reverse 4k emulation crap is not useful since we are way past Windows XP at this
        // point.

        // TODO: Based on other SCSI reported information, read the ATA logs that the information should come from to
        // compare and make sure it matches, or is close (>= what SCSI reported)
        //       This should be device statistics, other identify data not checked above, command/feature support, EPC
        //       settings, etc
    }
    return satSupported;
}
...
static bool test_Legacy_ATA_Passthrough(ptrPassthroughTestParams inputs, ptrScsiDevInformation scsiInformation)
{
    ...
    set_Console_Colors(true, HEADING_COLOR);
    print_str("\n====================================================================\n");
    print_str("Testing For Legacy ATA Passthrough Support With Known Legacy Methods\n");
    print_str("====================================================================\n");
    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    set_Console_Colors(true, WARNING_COLOR);
    print_str("WARNING! Legacy ATA Passthrough methods use vendor unique operation codes!\n");
    print_str("         Sometimes these operation code will cause problems on devices where\n");
    print_str("         these operation codes have different meanings!\n");
    print_str("         In some rare cases, this can render the adapter or device unusable!\n");
    print_str("         Do not run these legacy methods on USB thumb-drives/flash drives!\n");
    print_str("         These methods may be ok to run on USB HDDs from about 2007 & earlier\n");
    print_str("         Newer devices should use SAT!\n");
    print_str("         Press CTRL-C to abort this test immediately and remove the legacy test option\n");
    print_str("         if you are unsure if you should proceed with the test!\n");
    for (uint8_t counter = UINT8_C(30); counter > 0; --counter)
    {
    ...
    // Now that the users have been warned, it's time to try the different legacy methods until we get a successful
    // identify command through to the device.
    inputs->device->drive_info.passThroughHacks.passthroughType =
        ATA_PASSTHROUGH_SAT +
        1; // go to the next passthrough type after SAT since SAT is the default since it is the standard.
    ...
    {
        set_Console_Colors(true, HACK_COLOR);
        print_str("HACK FOUND: passthrough type seems to be \n"); // no passthrough available at all
        ...
        case ATA_PASSTHROUGH_PSP:
            ...
            break;
        default:
            print_str("Unknown legacy device type! This means the passthrough test code needs updating!\n");
            break;
        }
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
...
static eReturnValues scsi_Max_Transfer_Length_Test(tDevice* device, uint32_t reportedMax, uint32_t reportedOptimal)
{
    ...
    {
        set_Console_Colors(true, ERROR_COLOR);
        printf("Fatal error, unable to allocate %" PRIu32
               " sectors worth of memory to perform SCSI pass-through test.\n",
               maxTestSizeBlocks);
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        return MEMORY_FAILURE;
    }
    // start at 1 sector, then double until 32k. From here, increment one sector at a time since most USB devices only
    // handle 32k properly.
    uint32_t      transferLengthSectors = UINT32_C(1);
    ...
    while (transferLengthSectors <= maxTestSizeBlocks && readResult == SUCCESS)
    {
        ...
        if (readResult == SUCCESS)
        {
            device->drive_info.passThroughHacks.scsiHacks.maxTransferLength =
                transferLengthSectors * device->drive_info.deviceBlockSize;
        }
...
static eReturnValues ata_PT_Read(tDevice* device, uint64_t lba, bool async, uint8_t* ptrData, uint32_t dataSize)
{
    ...
    {
        if (device->drive_info.ata_Options.fourtyEightBitAddressFeatureSetSupported &&
            !device->drive_info.passThroughHacks.ataPTHacks.ata28BitOnly)
        {
            ...
            else
            {
                    ...
                    device->drive_info.passThroughHacks.ataPTHacks.dmaNotSupported)
                {
                        ...
                        device->drive_info.ata_Options.isParallelTransport)
                    {
                        ...
                        if (device->drive_info.ata_Options.chsModeOnly)
                        {
                            ...
                            {
                                ret = ata_Legacy_Read_Multiple_CHS(device, cylinder, head, sector, ptrData, sectors,
                                                                   dataSize, true);
                            }
                    ...
                    else
                    {
                        if (device->drive_info.ata_Options.chsModeOnly)
                        {
                            ...
                            if (SUCCESS == convert_LBA_To_CHS(device, C_CAST(uint32_t, lba), &cylinder, &head, &sector))
                            {
                                ret = ata_Legacy_Read_Sectors_CHS(device, cylinder, head, sector, ptrData, sectors,
                                                                  dataSize, true);
                            }
                            else // Couldn't convert or the LBA is greater than the current CHS mode
                            {
                ...
                else
                {
                    if (device->drive_info.ata_Options.chsModeOnly)
                    {
                        ...
                        if (SUCCESS == convert_LBA_To_CHS(device, C_CAST(uint32_t, lba), &cylinder, &head, &sector))
                        {
                            ret = ata_Legacy_Read_DMA_CHS(device, cylinder, head, sector, ptrData, sectors, dataSize,
                                                          true);
                        }
                        else // Couldn't convert or the LBA is greater than the current CHS mode
                        {
                ...
                else
                {
                    ...
                    else
                    {
                        if (device->drive_info.ata_Options.chsModeOnly)
                        {
                            ...
                            if (SUCCESS == convert_LBA_To_CHS(device, C_CAST(uint32_t, lba), &cylinder, &head, &sector))
                            {
                                ret = ata_Legacy_Read_DMA_CHS(device, cylinder, head, sector, ptrData, sectors,
                                                              dataSize, false);
                            }
                            else // Couldn't convert or the LBA is greater than the current CHS mode
                            {
...
static eReturnValues ata_Passthrough_Max_Transfer_Length_Test(tDevice* device,
                                                              uint32_t scsiReportedMax,
                                                              uint32_t scsiReportedOptimal)
{
    ...
    size_t dataBufSize =
        uint32_to_sizet(maxTestSizeBlocks) * uint32_to_sizet(device->drive_info.bridge_info.childDeviceBlockSize);
    uint8_t* data = M_REINTERPRET_CAST(
        uint8_t*, safe_calloc_aligned(dataBufSize, sizeof(uint8_t), device->os_info.minimumAlignment));
    set_Console_Colors(true, HEADING_COLOR);
    print_str("\n=============================================\n");
    print_str("Testing ATA Pass-through Maximum transfer size\n");
    print_str("=============================================\n");
    set_Console_Colors(true, NOTE_COLOR);
    printf("NOTE: This is currently limited to %" PRIu32 " sectors for now\n", maxTestSizeBlocks);
    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
    if (!data)
    {
        set_Console_Colors(true, ERROR_COLOR);
        printf("Fatal error, unable to allocate %" PRIu32
               " sectors worth of memory to perform ATA pass-through test.\n",
               maxTestSizeBlocks);
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        return MEMORY_FAILURE;
    }
    ...
    while (transferLengthSectors <= maxTestSizeBlocks && readResult == SUCCESS)
    {
        readResult = ata_PT_Read(device, 0, false, data,
                                 transferLengthSectors * device->drive_info.bridge_info.childDeviceBlockSize);
        scsi_Test_Unit_Ready(device, M_NULLPTR);
        if (readResult == SUCCESS)
        {
...
{
    eReturnValues ret = SUCCESS;
    print_str("Performing Pass-through test. \n");
    print_str("If at any point during the test, a crash, a hang, or the device\n");
    print_str("seems to stop responding to any normal request, save all previously\n");
    print_str("reported hacks, stop the tool, reset the device, then restart\n");
    print_str("the test, skipping to another part of the test to avoid the same issue.\n");
    print_str("All discovered hacks will be necessary in order to fully communicate\n");
    print_str("with the end target device to the highest capabilities of the adapter or\n");
    print_str("bridge performing the translations.\n\n");
    ...
    if (inputs && inputs->device)
    {
        ...
        {
            if (strncmp(scsiInformation.inquiryData.vendorId, "NVMe", 4) == 0 ||
                inputs->suspectedDriveType == NVME_DRIVE) // NVMe
            {
                set_Console_Colors(true, HEADING_COLOR);
                print_str("\n==================================================\n");
                print_str("Checking Vendor Specific NVMe-passthrough commands\n");
                print_str("==================================================\n");
                set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                ...
                if (inputs->device->drive_info.interface_type != NVME_INTERFACE)
                {
                    ...
                    inputs->device->drive_info.passThroughHacks.passthroughType =
                        NVME_PASSTHROUGH_JMICRON; // first NVMe passthrough in the list
                    ...
                    case NVME_PASSTHROUGH_JMICRON:
                        set_Console_Colors(true, HACK_COLOR);
                        print_str("HACK FOUND: JMNVME\n");
                        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        break;
                    case NVME_PASSTHROUGH_ASMEDIA_BASIC:
                        set_Console_Colors(true, HACK_COLOR);
                        print_str("HACK FOUND: ASMNVMEBASIC\n");
                        print_str("HACK FOUND: IDGLP\n");
                        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        inputs->device->drive_info.passThroughHacks.nvmePTHacks.limitedPassthroughCapabilities = true;
                        inputs->device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported
                            .identifyController = true;
                        inputs->device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported
                            .identifyNamespace = true;
                        inputs->device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported.getLogPage =
                            true;
                        break;
                    case NVME_PASSTHROUGH_ASMEDIA:
                        set_Console_Colors(true, HACK_COLOR);
                        print_str("HACK FOUND: ASMNVME\n");
                        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                        break;
                    default:
                        print_str("No NVMe pass-through detected for this device!\n");
                        break;
                    }
                    // TODO: Based on passthrough type, do more testing for max xferlength, etc
                }
            }
            else if (strncmp(scsiInformation.inquiryData.vendorId, "ATA", 3) == 0 ||
                     inputs->suspectedDriveType == ATA_DRIVE || inputs->allowLegacyATAPTTest) // ATA
            {
                // TODO: need to move this SAT testing to another function.
                if (!test_SAT_Capabilities(inputs, &scsiInformation))
                {
                    set_Console_Colors(true, ERROR_COLOR);
                    print_str("ERROR: SAT Pass-through failed both 12B and 16B CDBs.\n");
                    set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
                ...
                if (inputs->device->drive_info.passThroughHacks.passthroughType != ATA_PASSTHROUGH_UNKNOWN)
                {
                    ata_Passthrough_Max_Transfer_Length_Test(inputs->device,
                                                             scsiInformation.vpdData.blockLimitsData.maximumXferLen,
                                                             scsiInformation.vpdData.blockLimitsData.optimalXferLen);
                }
            }
        }
        ...
        {
            set_Console_Colors(true, NOTE_COLOR);
            printf("NOTE: Attempting passthrough CDBs for SAT or a vendor unique methods has been disabled and is "
                   "being skipped.\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        }
        ...
        set_Console_Colors(true, HEADING_COLOR);
        print_str("\n\n==================\n");
        print_str("Final Test Results\n");
        print_str("==================\n");
        set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
        print_str("SEND THIS INFO BELOW TO seaboard@seagate.com:\n");
        ...
        if (inputs->device->drive_info.adapter_info.infoType != ADAPTER_INFO_UNKNOWN)
        {
            ...
            {
                print_str("\tVendor ID: Did not get vendor ID from OS.\n");
            }
            ...
            {
                print_str("\tProduct ID: Did not get vendor ID from OS.\n");
            }
            ...
            {
                print_str("\tRevision: Did not get vendor ID from OS.\n");
            }
        }
        ...
        if (!inputs->device->drive_info.passThroughHacks.scsiHacks.preSCSI2InqData)
        {
            ...
            if (scsiInformation.vpdData.gotDeviceIDVPDPage)
            {
                print_str("\tDevice IDs:\n");
                for (uint8_t iter = UINT8_C(0); iter < scsiInformation.vpdData.designatorCount; ++iter)
                {
                    if (scsiInformation.vpdData.designators[iter].valid)
                    {
                        print_str("\t\t");
                        for (uint16_t desIter = UINT16_C(0);
                             desIter < scsiInformation.vpdData.designators[iter].designatorLength &&
                             desIter < UINT16_C(16);
                             ++desIter)
                        {
        ...
        {
            print_str("\tDevice information could not be detected by this tool since it was reported in a legacy\n");
            print_str("\tvendor specific format from old SCSI or CCS (pre-SCSI2) standards. Detecting this must\n");
        ...
        if (inputs->device->drive_info.passThroughHacks.passthroughType < ATA_PASSTHROUGH_UNKNOWN &&
            (inputs->device->drive_info.drive_type != NVME_DRIVE ||
             (inputs->suspectedDriveTypeProvidedByUser && inputs->suspectedDriveType != NVME_DRIVE)) &&
            inputs->device->drive_info.passThroughHacks.ataPTHacks.maxTransferLength <
                (MAX_ATA_SECTORS_TO_TEST * inputs->device->drive_info.bridge_info.childDeviceBlockSize))
        {
             ...
             (inputs->suspectedDriveTypeProvidedByUser && inputs->suspectedDriveType != NVME_DRIVE)))
        {
            ...
            case ATA_PASSTHROUGH_SAT:
                ...
                break;
            case ATA_PASSTHROUGH_CYPRESS:
                print_str(" APTCYPRESS,");
                break;
            case ATA_PASSTHROUGH_PROLIFIC:
                print_str(" APTPROLIFIC,");
                break;
            case ATA_PASSTHROUGH_TI:
                print_str(" APTTI,");
                break;
            ...
            if (inputs->device->drive_info.passThroughHacks.ataPTHacks.maxTransferLength <
                (MAX_ATA_SECTORS_TO_TEST * inputs->device->drive_info.bridge_info.childDeviceBlockSize))
            {
        ...
        else if ((inputs->suspectedDriveTypeProvidedByUser && inputs->suspectedDriveType == NVME_DRIVE) ||
                 (inputs->device->drive_info.passThroughHacks.passthroughType >= NVME_PASSTHROUGH_JMICRON &&
                  inputs->device->drive_info.passThroughHacks.passthroughType < NVME_PASSTHROUGH_UNKNOWN))
        {
        ...
        // If a translator is found or suspected, mention pursuing a standardized passthrough if not already using one.
        // If one doesn't exist, suggest creating one.
        uint8_t recommendationCounter = UINT8_C(1);
        printf("%" PRIu8 "\tTest this device with this tool in multiple OS's. Linux and Windows testing should be done "
               "to find highest compatibility.\n",
        ...
        if (inputs->device->drive_info.passThroughHacks.testUnitReadyAfterAnyCommandFailure)
        {
            printf("%" PRIu8 "\tImprove error handling for unknown commands. This device takes longer\n",
                   recommendationCounter);
            print_str("\t    and longer to respond with each unknown command.\n");
            ++recommendationCounter;
        }
        if (inputs->device->drive_info.passThroughHacks.passthroughType != ATA_PASSTHROUGH_SAT &&
            inputs->device->drive_info.passThroughHacks.passthroughType != ATA_PASSTHROUGH_UNKNOWN)
        {
            printf("%" PRIu8 "\tUse a standardized passthrough CDB. If a standard does not exist, take it to\n",
                   recommendationCounter);
            print_str("\tone of the spec committees and get one created! If they won't do it, at least\n");
            print_str("\tmake it an open document that anyone else can find and use!\n");
            ++recommendationCounter;
        }
        if (!inputs->testPotentiallyDeviceHangingCommands)
        {
            printf("%" PRIu8 "\tRerun this test to test for commands that are known to hang some devices to ensure\n",
                   recommendationCounter);
            print_str("\tall hacks are known for this device to perform properly...if the firmware can be\n");
            print_str("\tfixed to prevent these hangs, that is the best solution!\n");
            ++recommendationCounter;
        }
#if defined(__linux__)
        if (inputs->device->drive_info.passThroughHacks.passthroughType == ATA_PASSTHROUGH_UNKNOWN)
        {
            printf("%" PRIu8 "\tThis device doesn't support passthrough OR this device is being blacklisted\n",
                   recommendationCounter);
            print_str("\tby the Linux kernel from issuing SAT ATA-Passthrough commands.\n");
            print_str("\tPlease test in another OS and see if this device still reports the same way.\n");
            print_str("\tIt may fail even if SAT ATA-Passthrough commands are allowed, but the device isn't\n");
            print_str("\thandling them correctly in UAS mode.\n");
            ++recommendationCounter;
        }
#endif
        if (scsiInformation.vpdData.gotBlockLimitsVPDPage)
        {
            ...
            if (inputs->device->drive_info.passThroughHacks.scsiHacks.maxTransferLength <
                (scsiInformation.vpdData.blockLimitsData.maximumXferLen * inputs->device->drive_info.deviceBlockSize))
            {
                printf("%" PRIu8
                       "\tThe maximum transfer length is less than was reported by the block limits VPD page!\n",
                       recommendationCounter);
                printf("\tThis page should report the maximum transfer length supported by the SCSI device or SCSI "
                       "translator\n");
                printf(
                    "\tThis should be a true maximum. If this seems low, retest on another OS to ensure it is not a\n");
                printf("\tlimitation on a specific OS. For example, it is fairly common to be limited to 64k in "
                       "Windows.\n");
                ++recommendationCounter;
            }
        }
             ...
             (inputs->suspectedDriveTypeProvidedByUser && inputs->suspectedDriveType != NVME_DRIVE)))
        {
            ...
            if (inputs->device->drive_info.ata_Options.dmaMode == ATA_DMA_MODE_NO_DMA)
            {
                if (inputs->device->drive_info.ata_Options.dmaSupported ||
                    inputs->device->drive_info.ata_Options.downloadMicrocodeDMASupported ||
                    inputs->device->drive_info.ata_Options.readBufferDMASupported ||
                    inputs->device->drive_info.ata_Options.readLogWriteLogDMASupported ||
                    inputs->device->drive_info.ata_Options.writeBufferDMASupported)
                {
                    printf(
                        "%" PRIu8
                        "\tThis device has no way to passthrough DMA mode commands. This should be fixed as DMA mode\n",
                        recommendationCounter);
                    printf("\tprovides better performance, and on some devices is necessary to get some data due to "
                           "PIO issues.\n");
            ...
            if (inputs->device->drive_info.passThroughHacks.ataPTHacks.multiSectorPIOWithMultipleMode)
            {
                printf(
                    ...
                    recommendationCounter);
                printf("\tbut this issue should be resolved by the driver/translator. This is likely a problem where "
                       "it is assumed\n");
                printf("\tthat all PIO transfers will be single sectors, which is an incorrect assumption, and the "
                       "device does not\n");
                printf(
                    "\tproperly handle the interrupts/PIO setup FISs between each sector of data being transferred.\n");
                printf("\tMulti-sector PIO transfers need to be supported for more compatibility with tools that read "
                       "logs or update firmware.\n");
                printf("\tMulti-sector PIO transfers should be supported without this workaround, which may still be "
                       "restrictive\n");
                printf("\tfor some cases. Multiple mode has been removed from newer ATA specifications, so this needs "
                       "supported without\n");
            ...
            else if (inputs->device->drive_info.passThroughHacks.ataPTHacks.singleSectorPIOOnly)
            {
                printf(
                    ...
                    recommendationCounter);
                printf("\tusing SMART read log or Read Log Ext for multiple 512B pages at a time. These will not be "
                       "accessible on this\n");
                printf("\tdevice. It is also an issue as device firmware may not be upgradeable since that is also a "
                ...
                inputs->device->drive_info.ata_Options.fourtyEightBitAddressFeatureSetSupported)
            {
                printf("%" PRIu8 "\tThis device can only issue 28bit commands, but supports 48bit addressing.\n",
                       recommendationCounter);
                print_str("\tThis means loss of capability through pass-through and needs to be resolved to allow\n");
                print_str("\tsending read, write, or read log ext commands to the device for more compatibility.\n");
                ++recommendationCounter;
            }
        }
    }
...
void utility_Usage(bool shortUsage)
{
    ...
    print_str("\tTypical use of this test tool usually requires running it twice:\n");
    printf("\t1. %s -d %s --%s --%s ata --%s sat\n", util_name, deviceHandleExample,
           RUN_PASSTHROUGH_TEST_LONG_OPT_STRING, PT_DRIVE_HINT_LONG_OPT_STRING, PT_PTTYPE_HINT_LONG_OPT_STRING);
    print_str("\tEither reboot or power cycle the adapter between runs\n");
    printf("\t2. %s -d %s --%s --%s ata --%s sat --%s all\n", util_name, deviceHandleExample,
           RUN_PASSTHROUGH_TEST_LONG_OPT_STRING, PT_DRIVE_HINT_LONG_OPT_STRING, PT_PTTYPE_HINT_LONG_OPT_STRING,
           ENABLE_HANG_COMMANDS_TEST_LONG_OPT_STRING);
    print_str("\tLogs from each run should be collected to share with the openSeaChest developers.\n");
    // TODO: another example for USB to NVMe adapters. There are a few other changes necessary to make them able to be
    // specified directly.
