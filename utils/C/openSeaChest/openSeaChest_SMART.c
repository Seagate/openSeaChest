...
int main(int argc, char* argv[])
{
        ...
        case 0:
            ...
            else if (strcmp(longopts[optionIndex].name, IDD_TEST_LONG_OPT_STRING) == 0)
            {
                ...
                else
                {
                    ...
                    uint32_t iddTestNumber = UINT32_C(0);
    ...
    if (!(DEVICE_INFO_FLAG || TEST_UNIT_READY_FLAG || LOWLEVEL_INFO_FLAG || SMART_CHECK_FLAG || SMART_ATTRIBUTES_FLAG ||
    ...
    for (uint32_t deviceIter = UINT32_C(0); deviceIter < DEVICE_LIST_COUNT; ++deviceIter)
    {
        ...
        if (RUN_IDD_FLAG)
        {
            ...
            if (family == SEAGATE)
            {
                if (VERBOSITY_QUIET < toolVerbosity)
                {
                    uint64_t iddTimeSeconds = UINT64_C(0);
                    uint8_t  hours          = UINT8_C(0);
                    uint8_t  minutes        = UINT8_C(0);
                    uint8_t  seconds        = UINT8_C(0);
                    get_Approximate_IDD_Time(&deviceList[deviceIter], C_CAST(eIDDTests, IDD_TEST_FLAG),
                                             &iddTimeSeconds);
...
void utility_Usage(bool shortUsage)
{
    ...
    printf("\t%s -d %s --%s --%s\n", util_name, deviceHandleExample, SHORT_DST_LONG_OPT_STRING,
           CAPTIVE_LONG_OPT_STRING);
    printf("\t%s -d %s --%s --%s\n", util_name, deviceHandleExample, CONVEYANCE_DST_LONG_OPT_STRING,
