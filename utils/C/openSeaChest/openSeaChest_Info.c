...
int32_t main(int argc, char *argv[])
{
    ...
    while (1) //changed to while 1 in order to parse multiple options when longs options are involved
    {
        ...
        case 0:
            ...
            if (strncmp(longopts[optionIndex].name, SMART_ATTRIBUTES_LONG_OPT_STRING, M_Min(strlen(longopts[optionIndex].name), strlen(SMART_ATTRIBUTES_LONG_OPT_STRING))) == 0)
            {
